...
- FULL ./test.sh must exit 0; bash scripts/rue fmt before finishing. Quote the banner AND the \`Tests finished: Pass N. Fail 0 ...\` tally in your meta. \`=== TEST SUITE: PASSED ===\` is asserted, not inferred, so it is trustworthy; a run the OOM killer or a disk-full host stopped prints \`=== TEST SUITE: INTERRUPTED (SIG..) ===\` instead (RUE-1782). The tally alone is NOT a verdict — Buck can return cached results, so a killed run can print a complete-looking \`Fail 0\` line and still not have finished. Anything other than PASSED means the run did not pass; say so plainly rather than claiming one. No scope creep: fix exactly the listed issues. Update any comments your change makes stale — comments are source of truth here. Do not allocate error codes outside your reserved range.

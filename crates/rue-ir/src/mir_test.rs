...
fn test_mir_factorial() {
    ...
    assert_eq!(mir_func.blocks.len(), 4); // entry, then, else, join

    // Check that we have branch and return terminators
    let has_branch = mir_func
        .blocks
        .iter()
        .any(|b| matches!(b.terminator, crate::mir::MirTerminator::Branch { .. }));
    let has_return = mir_func
        .blocks
        .iter()
        .any(|b| matches!(b.terminator, crate::mir::MirTerminator::Return { .. }));
...
fn test_mir_unary_ops() {
    ...
    let has_unary = mir_func.blocks.iter().any(|block| {
        block.statements.iter().any(|stmt| match stmt {
            crate::mir::MirStatement::Assign { value, .. } => {
                matches!(value, crate::mir::MirValue::UnaryOp { .. })
            }
        })
    });
    assert!(has_unary);

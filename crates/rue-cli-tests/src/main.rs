...
/// - `ffi_call_struct_exports() -> long` — a C caller that invokes three Rue
///   exports across the crossings the two conventions disagree about: an 8-byte
///   `@repr(c)` struct by value and returned by value, a 24-byte struct (byval
...
fn synthesize_answer_archive(target: Target) -> TestResult<Vec<u8>> {
    ...
    // The aggregate-crossing export driver. `clang -O2 -c` (x86-64) and
    // `clang --target=aarch64-linux-gnu -O1 -c` produced the `.text` below for:
    //
    //   typedef struct { int a; int b; } Pair;      /* 8 bytes  */
    //   typedef struct { long a, b, c; } Triple;    /* 24 bytes */
    //   long ffi_call_struct_exports(void) {
    //       Pair p = {3, 5};
    //       Pair q = rue_pair_swap(p);              /* {6, 5}       */
    //       Triple t = {10, 20, 30};
    //       Triple u = rue_triple_rotate(t);        /* {30, 10, 20} */
    //       long s = rue_sum9(1,2,3,4,5,6,7,8,9);   /* 285          */
    //       long e = ffi_sret_echo_ok();            /* 1            */
    //       long v = (long)q.a * 100 + q.b;         /* 605          */
    //       v = v * 1000 + (u.a * 10 + u.b);        /* 310          */
    //       v = v * 1000 + (u.c * 10 + e);          /* 201          */
    //       v = v * 1000 + s;                       /* 285          */
    //       return v;                               /* 605310201285 */
    //   }
    //
    // The three exports exercise the three crossings that disagree between the
    // conventions: an 8-byte struct in one register each way (the native side
    // passes and returns it indirectly, through its compact image); a 24-byte
    // struct, byval-on-stack under SysV and by-reference under AAPCS64, returned
    // through caller storage; and nine scalars, whose tail is stacked under both
    // rows.
    let (call_struct_exports_code, call_struct_exports_relocs): (Vec<u8>, Vec<(u64, &str)>) =
        match target.arch() {
            Arch::X86_64 => (
                vec![
                    ,
                ],
                vec![
                    (18, "rue_pair_swap"),
                    (77, "rue_triple_rotate"),
                    (140, "rue_sum9"),
                    (148, "ffi_sret_echo_ok"),
                ],
            ),
            Arch::Aarch64 => (
                vec![
                    ,
                ],
                vec![
                    (28, "rue_pair_swap"),
                    (64, "rue_triple_rotate"),
                    (108, "rue_sum9"),
                    (116, "ffi_sret_echo_ok"),
                ],
            ),
        };
    // The SysV `rax` echo of the hidden result pointer, which no C compiler
    // exposes: call the 24-byte-returning export with a known caller buffer and
    // read the struct back *through the returned pointer*, answering 1 only when
    // `rax` really is the buffer address and the bytes behind it are the
    // rotation. Hand-written assembly, assembled by `clang -c`:
    //
    //   ffi_sret_echo_ok:
    //           pushq %rbp ; movq %rsp,%rbp ; subq $64,%rsp
    //           movq $10,0(%rsp) ; movq $20,8(%rsp) ; movq $30,16(%rsp)
    //           leaq -24(%rbp),%rdi ; callq rue_triple_rotate
    //           leaq -24(%rbp),%rcx ; cmpq %rcx,%rax ; jne .Lfail
    //           cmpq $30,0(%rax) ; jne .Lfail
    //           cmpq $10,8(%rax) ; jne .Lfail
    //           cmpq $20,16(%rax) ; jne .Lfail
    //           movl $1,%eax ; jmp .Ldone
    //   .Lfail: xorl %eax,%eax
    //   .Ldone: movq %rbp,%rsp ; popq %rbp ; retq
    //
    // AAPCS64 has no echo to check -- its hidden pointer travels in the
    // dedicated `x8`, which the caller never reads back -- so the AArch64 member
    // answers 1 unconditionally and the encoded result stays comparable across
    // both rows. The `x8` path itself is exercised by `ffi_call_struct_exports`,
    // whose compiler-generated caller uses it.
    let (sret_echo_code, sret_echo_relocs): (Vec<u8>, Vec<(u64, &str)>) = match target.arch() {
        Arch::X86_64 => (
            vec![
                0x55, 0x48, 0x89, 0xE5, 0x48, 0x83, 0xEC, 0x40, 0x48, 0xC7, 0x04, 0x24, 0x0A, 0x00,
                0x00, 0x00, 0x48, 0xC7, 0x44, 0x24, 0x08, 0x14, 0x00, 0x00, 0x00, 0x48, 0xC7, 0x44,
                0x24, 0x10, 0x1E, 0x00, 0x00, 0x00, 0x48, 0x8D, 0x7D, 0xE8, 0xE8, 0x00, 0x00, 0x00,
                0x00, 0x48, 0x8D, 0x4D, 0xE8, 0x48, 0x39, 0xC8, 0x75, 0x1B, 0x48, 0x83, 0x38, 0x1E,
                0x75, 0x15, 0x48, 0x83, 0x78, 0x08, 0x0A, 0x75, 0x0E, 0x48, 0x83, 0x78, 0x10, 0x14,
                0x75, 0x07, 0xB8, 0x01, 0x00, 0x00, 0x00, 0xEB, 0x02, 0x31, 0xC0, 0x48, 0x89, 0xEC,
                0x5D, 0xC3,
            ],
            vec![(39, "rue_triple_rotate")],
        ),
        // movz w0, #1 ; ret
        Arch::Aarch64 => (

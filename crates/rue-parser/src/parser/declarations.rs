...
impl Parser {
    ...
    pub(super) fn item(&mut self) -> PResult<Item> {
        ...
        match item_start {
            ...
            Some(recovery::ItemStart::Enum) => self
                ...
                .map(Item::Enum),
            Some(recovery::ItemStart::Interface) => self
            ...
            // 6.8:9) is the only item that begins with a type rather than a
            // keyword. It takes neither directives nor a visibility modifier.
            kind if directives.is_empty()
                && visibility == Visibility::Private
                && !self.at_test_item()
                && self.starts_conformance_subject(self.kind()) =>
            {
    ...
    /// `[pub] const Name = Type;` (spec 6.8:2, `struct_assoc_type`).
    ...
    /// Whether the current token is the contextual keyword `is` (spec 6.8:9).
    ...
    /// Parse `Type is Interface + Other;` (spec 6.8:2, `conformance_decl`).
    ...
    /// Parse a non-empty `+`-separated interface list (spec 6.8:2,
    ...
    /// signature `fn name(...) [-> result];` (spec 6.8:2, `interface_member`).
    ...
    pub(super) fn param(&mut self) -> PResult<Param> {
        ...
        self.expect(TokenKind::Colon)?;
        let ty = self.ty()?;
        // A composed interface bound `comptime T: A + B` (spec 6.8:14) is the
...
mod tests {
    ...
    fn parses_interface_declarations_with_every_requirement_form() {
        ...
        assert!(methods[4].place_return.is_none());
        assert!(interface.span.end > interface.span.start);
    }

    #[test]
    fn rejects_unsupported_interface_requirement_forms() {
        assert!(!parses("interface I { fn f(comptime T: type); }"));
        assert!(!parses("interface I { fn f(borrow self) -> borrow i64; }"));
    }

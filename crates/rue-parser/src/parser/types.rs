...
impl Parser {
    ...
    pub(super) fn primitive_spur(&self, kind: TokenKind) -> Option<Spur> {
        Some(match kind {
            ...
            // `comptime_float` is deliberately not a primitive here: it is a
            // type the implementation infers and no program may name (spec
            // 3.12:3), so it stays an ordinary identifier and is reported like
            // any other unknown name, exactly as `comptime_int` is (RUE-1076).
            TokenKind::F32 => self.syms.f32,
            TokenKind::F64 => self.syms.f64,
            _ => return None,
        })
    }
    ...
    /// interface requirement continues with `;` (spec 6.8:5).

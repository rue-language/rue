...
mod tests {
    ...
    fn collect_toml_sources(value: &toml::Value, path: &str, out: &mut Vec<TomlSource>) {
        match value {
                    ...
                    if key == "source" {
                        ...
                        if let Some(files) = value.as_array() {
                            for (index, file) in files.iter().enumerate() {
                                if let Some(source) =
                                    file.get("source").and_then(toml::Value::as_str)
                                {
                                    out.push(TomlSource {
                                        locator: format!("{child}[{index}].source"),
                                        source: source.to_owned(),
                                    });
                                }
                            }
                        ...
                        collect_toml_sources(value, &child, out);
                    }
                }
            }
            toml::Value::Array(values) => {
                for (index, value) in values.iter().enumerate() {
                    collect_toml_sources(value, &format!("{path}[{index}]"), out);
                }
            }
            _ => {}
        }
    ...
    fn toml_files(directory: &Path, out: &mut Vec<PathBuf>) {
    ...
    fn diagnostic_fingerprint(errors: &CompileErrors) -> String {
        ...
        for error in errors.iter() {
            text(&mut hasher, &format!("{:?}", error.kind));
            text(&mut hasher, &error.kind.to_string());
            match error.span() {
                Some(value) => {
                    hasher.update([1]);
                    span(&mut hasher, value);
                }
            ...
            for label in &diagnostic.labels {
                text(&mut hasher, &label.message);
                span(&mut hasher, label.span);
            }
            hasher.update((diagnostic.notes.len() as u64).to_le_bytes());
            for note in &diagnostic.notes {
                text(&mut hasher, &note.0);
            }
            hasher.update((diagnostic.helps.len() as u64).to_le_bytes());
            for help in &diagnostic.helps {
                text(&mut hasher, &help.0);
            }
            hasher.update((diagnostic.suggestions.len() as u64).to_le_bytes());
            for suggestion in &diagnostic.suggestions {
                text(&mut hasher, &suggestion.message);
                span(&mut hasher, suggestion.span);
                text(&mut hasher, &suggestion.replacement);
    ...
    fn feed_snapshot_text(hasher: &mut Sha256, value: &str) {
        hasher.update((value.len() as u64).to_le_bytes());
        hasher.update(value.as_bytes());
    }

    #[test]
    fn toml_diagnostic_corpus_matches_snapshot() {
        ...
        let mut accepted_ast_hasher = Sha256::new();
        let mut rejected = 0usize;
        let mut lex_invalid = 0usize;
        let mut rows = Vec::new();
        for file in files {
            let relative = file.strip_prefix(&root).unwrap_or(&file).display();
            let value: toml::Value = toml::from_str(&fs::read_to_string(&file).unwrap()).unwrap();
            let mut sources = Vec::new();
            collect_toml_sources(&value, "", &mut sources);
            for fixture in sources {
                let key = format!("{relative}#{}", fixture.locator);
                let Ok((tokens, interner)) = Lexer::new(&fixture.source).tokenize() else {
                    lex_invalid += 1;
                    continue;
                };
                match Parser::new(tokens, interner).parse() {
                    Ok((ast, _)) => {
                        accepted += 1;
                        for value in [&key, &fixture.source, &format!("{ast:?}")] {
                            feed_snapshot_text(&mut accepted_ast_hasher, value);
                        }
                    }
                    Err(errors) => {
                        rejected += 1;
    ...
    #[test]
    fn diagnostic_snapshot_fingerprints_detect_fixture_edits() {
        assert_ne!(
            source_fingerprint("let x = 1;"),
            source_fingerprint("let x = 2;")
        );

        let accepted = |source: &str| {
            let mut hasher = Sha256::new();
            for value in ["fixture-key", source, "Ast { items: [] }"] {
                feed_snapshot_text(&mut hasher, value);
            }
            hasher.finalize()
        };
        assert_ne!(accepted("let x = 1;"), accepted("let x = 2;"));
    }
}

...
pub enum Item {
    ...
    Struct(StructDecl),
    Enum(EnumDecl),
    /// An interface declaration (spec 6.8, `--preview interfaces`).
    Interface(InterfaceDecl),
    /// A freestanding conformance assertion `Type is Interface + Other;`
    /// (spec 6.8:9, `--preview interfaces`).
...
pub struct StructDecl {
    ...
    /// (spec 6.8:9). Empty for a struct without a header assertion.
    pub conformances: Vec<TypeExpr>,
    /// Struct fields
    pub fields: Vec<FieldDecl>,
    /// Associated type declarations `pub const Name = Type;` that satisfy an
    /// interface's type-valued associated constant requirements (spec 6.8:10).
...
/// `pub const Element = i64;` (spec 6.8:2, `struct_assoc_type`).
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// An interface declaration (spec 6.8).
///
/// ```rue
...
pub struct InterfaceDecl {
    ...
    /// (spec 6.8:7). Empty when the interface refines nothing.
...
/// One member requirement of an interface (spec 6.8:5).
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// requirement `fn name(params) -> ret;` (spec 6.8:5).
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// A freestanding conformance assertion `Type is Interface + Other;`
/// (spec 6.8:9). The subject may be any type expression, including a
...
pub struct Param {
    ...
    /// (`comptime T: Equatable + Sequence`, spec 6.8:14) this is the first

...
use crate::hir_to_mir::MirBuilder;
use rue_ir::hir::*;
use rue_ir::mir::*;
...
fn test_lower_if_expression() {
    ...
    // Should have exactly 3 blocks: entry, then, else (join block optimized away since both branches return)
...
#[test]
fn test_if_expression_join_block_optimization() {
    // Test that if-expressions are optimized differently based on context:
    // - Function body: both branches return -> 3 blocks (no join)
    // - Nested expression: branches don't return -> 4 blocks (with join)

    // Case 1: If-expression as function body (optimized)
    let func_body_if = make_simple_function(
        "test_if_optimized",
        HirExpr::If {
            cond: Box::new(HirExpr::Literal {
                lit: HirLiteral::Bool(true),
    ...
    let mir_optimized = MirBuilder::lower_function(&func_body_if);
    // Should have 3 blocks: entry, then, else (join block optimized away)
    assert_eq!(mir_optimized.blocks.len(), 3);

    // Case 2: If-expression nested in a larger expression (not optimized)
    let func_nested_if = HirFunction {
        name: "test_if_nested".to_string(),
        params: vec![],
        return_type: RueType::I32,
        body: HirBlock {
            statements: vec![],
    ...
    let mir_nested = MirBuilder::lower_function(&func_nested_if);

    // Should have 4 blocks: entry, then, else, join (join block needed for the addition)
    assert_eq!(mir_nested.blocks.len(), 4);
}

...
impl<'a> Lowering<'a> {
    ...
    /// Patch stack allocation instructions with the actual required stack space
    pub fn patch_stack_allocation(
        instructions: &mut [MachineInstr],
        allocator: &RegisterAllocator,
    ) {
        // Calculate the required stack space for spills
        let spill_space = allocator.get_stack_space_needed();

        // Ensure 16-byte alignment for System V ABI
        let aligned_size = if spill_space == 0 {
            0
        } else {
            // Round up to next 16-byte boundary
            spill_space.div_ceil(16) * 16
        };

        // Find and patch AllocStack instructions with size 0 (our placeholder)

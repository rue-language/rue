...
pub struct RirParam {
    ...
    /// first interface of the bound (spec 6.8:14); the interface/`type`
...
pub enum RirStructuralPathSegment {
    ...
    /// or freestanding assertion (spec 6.8:7, 6.8:9).
    Conformance(u32),
...
pub enum InstData {
    ...
    StructDecl {
        ...
        is_linear: bool,
        /// Whether this declaration is an `interface` (spec 6.8) rather than
        ...
        /// 6.8:9), or the refined interfaces of an interface (spec 6.8:7).
        conformances: RirInterfaceRefsRange,
        /// Associated type declarations `pub const Name = Type;` (spec 6.8:10)
    ...
    /// (spec 6.8:9). Header assertions are recorded on the `StructDecl`
    /// instead.
    ConformanceDecl {

...
fn legacy_convenience_mutation_instructions_are_contract_failures() {
    for opcode in ["FieldSet", "IndexSet", "ParamFieldSet", "ParamIndexSet"] {
        let mut state = compile_to_cfg(
            ...
            fn main() -> i32 {
                consume_pair(Pair { a: 1, b: 2 }) + consume_array([3, 4])
            }",
        )
        .expect("legacy-mutation probe must compile");
        let main_index = state
            .functions
            .iter()
            .position(|function| function.cfg.fn_name() == "main")
            .expect("main CFG");
        let (init, struct_id, field, array_type, index) = {
            let cfg = &state.functions[main_index].cfg;
            let (init, struct_id, field) = cfg
                .blocks()
                .iter()
                .flat_map(|block| block.insts.iter().copied())
                .find_map(|value| {
                    ...
                    Some((value, struct_id, cfg.get_extra(fields_start, fields_len)[0]))
                })
                .expect("Pair StructInit");
            let (array_type, index) = cfg
                .blocks()
                .iter()
                .flat_map(|block| block.insts.iter().copied())
                .find_map(|value| {
                    ...
                    Some((
                        ...
                        cfg.get_extra(elements_start, elements_len)[0],
                    ))
                })
                .expect("array initializer");
            (init, struct_id, field, array_type, index)
        };
        state.functions[main_index].cfg.get_inst_mut(init).data = match opcode {
            "FieldSet" => CfgInstData::FieldSet {
                slot: 0,
                struct_id,
                field_index: 0,
                value: field,
            },
            "IndexSet" => CfgInstData::IndexSet {
                slot: 0,
                array_type,
                index,
                value: field,
            },
            "ParamFieldSet" => CfgInstData::ParamFieldSet {
                param_slot: 0,
                inner_offset: 1,
                struct_id,
                field_index: 0,
            ...
            "ParamIndexSet" => CfgInstData::ParamIndexSet {
                param_slot: 0,
                array_type,
                index,
                value: field,
        ...
        let unsupported = match run_state_with_budget(state, STEP_BUDGET) {
            Err(unsupported) => unsupported,
            Ok(outcome) => panic!("{opcode} unexpectedly ran to completion: {outcome:?}"),
        };
        assert_eq!(
            unsupported.kind(),
        ...
        assert_eq!(unsupported.model_gap(), None, "{opcode}");
    }
}

...
struct TypeInternPoolInner {
    ...
    /// Structs that are `interface` shells (spec 6.8): an interface lowers to
    ...
    /// required (spec 6.8:18), and conformance verification reads its
...
impl TypeInternPool {
    ...
    /// Record that a struct is an `interface` shell (spec 6.8). Set when the
    /// shell's durable nominal is registered; read by body type resolution
    /// (an interface is not a type, spec 6.8:18) and by conformance
    /// verification.
...
impl FrozenTypeInternPool {
    ...
    /// Whether a struct is an `interface` shell (spec 6.8).

...
impl TypeInternPoolInner {
    ...
    fn derive_overlay(&self) -> Self {
        ...
        Self {
            ...
            struct_lang_items: self.struct_lang_items.clone(),
            lang_item_structs: self.lang_item_structs.clone(),
...
impl FrozenTypeInternPool {
    ...
    pub fn validate_for_success(&self) -> Result<(), TypeValidationError> {
        for (index, entry) in self.inner.types.iter().enumerate() {
            debug_assert!(self.inner.base.is_none(), "a frozen pool is flat");

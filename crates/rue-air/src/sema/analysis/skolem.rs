...
//! (spec 6.8:19–6.8:22, preview `interfaces`).
//!
...
//! requirements of the bound (spec 6.8:20). The compiler mints the skolem
...
//! interfaces disagree about one member (spec 6.8:21).
//!
...
impl<H: OrdinaryBodyAnalysisHost> OrdinaryBodyEngine<'_, H> {
    ...
    /// arguments before its body is analyzed (spec 6.8:20). Non-skolem
    ...
    fn prepare_skolem(&mut self, ty: Type) -> CompileResult<()> {
        ...
        // assertions are the bound (spec 6.8:20) and satisfy every nested
    ...
    /// refine, instantiated for the skolem `ty` (spec 6.8:20), with the
    /// skolem's associated types. Two interfaces may declare the same
    /// member only with the same signature (spec 6.8:21).
...
/// the same comparison conformance verification makes (spec 6.8:10).
fn same_signature(left: &RequirementSignature, right: &RequirementSignature) -> bool {
    left.has_self == right.has_self
        ...
        && left.returns_borrow == right.returns_borrow
        && left.returns_inout == right.returns_inout
        && left.params.len() == right.params.len()
        && left.params.iter().zip(right.params.iter()).all(
            |(
                (_, left_mode, left_ty, left_comptime),
                (_, right_mode, right_ty, right_comptime),
            )| {
                left_mode == right_mode && left_ty == right_ty && left_comptime == right_comptime
            },
        )
}
...
mod tests {
    ...
    /// with the skolem as the type argument (spec 6.8:19).
    fn check(
    ...
    fn a_composed_bound_unions_the_requirements() {
        ...
        check(&mut fixture, &[show, twice], "x.show() + x.twice()")
            .expect("the skolem of `Show + Twice` has both members (spec 6.8:20)");
    }
    ...
    fn refinement_is_walked_when_synthesizing_members() {
        ...
        check(&mut fixture, &[louder], "x.show() + x.loud() + x.louder()")
            .expect("every refined interface's requirements are members (spec 6.8:20)");
    }
    ...
    fn conflicting_requirements_are_reported_at_the_parameter() {
        ...
        let error = check(&mut fixture, &[sized, counted], "0")
            .err()
            .expect("`len` differs between the two interfaces (spec 6.8:21)");
    ...
    fn agreeing_requirements_contribute_one_member() {
        ...
        check(&mut fixture, &[sized, counted], "@intCast(x.len())")
            .expect("one signature declared twice is one member (spec 6.8:21)");
    }
}

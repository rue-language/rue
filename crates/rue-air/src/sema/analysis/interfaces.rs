//! Interface bounds and conformance verification (spec 6.8, preview
//! `interfaces`).
//!
...
//! conforms (spec 6.8:15), and verifies the conformance assertions that
//! decision relied on against the type's inherent members (spec 6.8:10).
//!
...
/// One unsatisfied requirement of a conformance assertion (spec 6.8:10),
...
impl<H: OrdinaryBodyAnalysisHost> OrdinaryBodyEngine<'_, H> {
    ...
    /// that binds it to `argument` (spec 6.8:15). A parameter without a bound
    ...
    pub(crate) fn check_interface_bounds(
        ...
        // without the preview (spec 6.8:25); every other bound is gated at
        // the call as at its declaration (spec 6.8:3).
        ...
        for bound in bounds {
            let Some(assertion) = self.assertion_satisfying(argument, bound) else {
                ...
                // Under a skolem check (spec 6.8:19) the argument is the
    ...
    /// refining it (spec 6.8:12), if the body can see one.
    ...
    /// `interface` and every interface it transitively refines (spec 6.8:7),
    ...
    /// members (spec 6.8:10), covering every interface the asserted one
    /// refines (spec 6.8:12). Each (subject, interface) pair is verified once
    /// per body: repeated assertions are one fact (spec 6.8:11), and a later
    ...
    fn requirement_failures(
        ...
        for name in &facts.method_requirements {
            ...
            // Compare the interned type identities directly.  The friendly
            // rendering is intentionally short (and may omit module paths),
            // so using it as the semantic key would make two distinct
            // nominal types named `Item` interchangeable.
            if !self.member_matches_requirement(&member, &required) {
                failures.push(RequirementFailure {
    ...
    fn member_matches_requirement(
        &self,
        member: &MethodCallInfo,
        required: &RequirementSignature,
    ) -> bool {
        if member.has_self != required.has_self
            || member.self_mode != required.self_mode
            || member.return_type != required.result
            || member.returns_borrow != required.returns_borrow
            || member.returns_inout != required.returns_inout
        {
            return false;
        }
        ...
        required.params.len() == types.len()
            && required
                .params
                .iter()
                .zip(modes.iter().zip(types.iter().zip(comptime.iter())))
                .all(
                    |(
                        (_, required_mode, required_ty, required_comptime),
                        (found_mode, (found_ty, found_comptime)),
                    )| {
                        required_mode == found_mode
                            && required_ty == found_ty
                            && required_comptime == found_comptime
                    },
                )
    }

    /// Render a requirement for E0303. Parameter names do not distinguish
    /// signatures; access modes, comptime markers, and result access do.
    /// Semantic agreement is decided by `member_matches_requirement`.
    fn render_requirement_signature(&self, name: &str, signature: &RequirementSignature) -> String {
        let params = signature
            .params
            .iter()
            .map(|(_, mode, ty, comptime)| (*mode, *ty, *comptime))
            .collect::<Vec<_>>();
        self.interface_signature_display(
            name,
            signature.has_self,
            signature.self_mode,
            &params,
            signature.result,
            signature.returns_borrow,
            signature.returns_inout,
        )
    }
    ...
    fn render_member_signature(&self, name: &str, member: &MethodCallInfo) -> String {
        ...
        let params = data
            .modes()
            .iter()
            .zip(data.types().iter().zip(data.comptime()))
            .map(|(mode, (ty, comptime))| (*mode, *ty, *comptime))
            .collect::<Vec<_>>();
        self.interface_signature_display(
            name,
            member.has_self,
            ...
            &params,
            member.return_type,
            member.returns_borrow,
            member.returns_inout,
        )
    }

    fn interface_signature_display(
        &self,
        name: &str,
        has_self: bool,
        self_mode: RirParamMode,
        params: &[(RirParamMode, Type, bool)],
        result: Type,
        returns_borrow: bool,
        returns_inout: bool,
    ) -> String {
        ...
        for (mode, ty, comptime) in params {
            parts.push(format!(
                "{}{}_: {}",
        ...
        if result != Type::UNIT {
            rendered.push_str(if returns_borrow {
                " -> borrow "
...
pub(super) mod interface_fixtures {
    ...
    /// bounded comptime type parameter (spec 6.8:14).
        ...
        fixture.declare_struct_with(
            ...
            StructShape {
                conformance: DurableConformanceFacts {
                    ...
                    assoc_types: assoc
                        .iter()
                        .map(|name| (Arc::from(*name), T::ComptimeType, true))
                        .collect(),
...
mod tests {
    ...
    fn missing_assertion_is_reported_at_the_argument() {
        ...
        let error = fixture
            .analyze(RENDER_MAIN, "main")
            .err()
            .expect("no assertion means no conformance (spec 6.8:15)");
        assert!(
            matches!(
                &error.kind,
                ErrorKind::InterfaceBoundNotSatisfied { ty, interface }
    ...
    fn bound_requires_the_interfaces_preview() {
        ...
        let error = fixture
            .analyze(RENDER_MAIN, "main")
            .err()
            .expect("a bound is gated (spec 6.8:3)");
        assert!(
            matches!(
                &error.kind,
    ...
    fn missing_associated_type_is_unsatisfied() {
        ...
        let error = missing
            ...
            .err()
            .expect("a missing associated type is unsatisfied (spec 6.8:10)");
        assert!(
    ...
    fn associated_type_is_substituted_into_requirements() {
        ...
        let range = fixture.declare_struct_with(
            ...
            StructShape {
                conformance: DurableConformanceFacts {
                    conformances: Arc::from([assertion(&sequence)]),
                    assoc_types: Arc::from([(Arc::from("Element"), T::I64, true)]),
                    ..DurableConformanceFacts::default()
                },
                ..StructShape::default()
            },
    ...
    fn refinement_is_walked_transitively_and_is_cycle_safe() {
        ...
        // on `Show` and verifies every interface on the way (spec 6.8:12).
    ...
    fn a_verified_assertion_answers_every_later_call_in_the_body() {
        ...
        declare_borrow_method(&mut fixture, &val, "show", Vec::new(), T::I64, None);
        // Two calls and a repeated assertion (spec 6.8:11): one verification,
        // one answer.

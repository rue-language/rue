...
impl<H: OrdinaryBodyAnalysisHost> OrdinaryBodyEngine<'_, H> {
    ...
    pub(super) fn analyze_addr_of_intrinsic(
        ...
        // Taking the address is a shared use of the root, whatever the
        // pointer's mutability: it may not overlap an exclusive accessor
        // result in the same full expression (6.6:10).
        let operand_span = self.body_rir_ref().get(operand).span;
        self.record_borrowed_place_use(operand, operand_span, ctx)?;

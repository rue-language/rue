...
/// The declaration facts of an `interface` shell (spec 6.8), read by
/// conformance verification. `parents` are the interfaces it refines
/// (spec 6.8:7); `assoc_requirements` are its `const Name: type;`
...
/// One conformance assertion for a subject type (spec 6.8:9): the asserted
...
/// associated-constant names (spec 6.8:10), in the vocabulary a subject's
/// inherent member is compared against.
#[derive(Debug, Clone)]
pub(crate) struct RequirementSignature {
    pub has_self: bool,
    pub self_mode: rue_rir::RirParamMode,
    pub params: Vec<(std::sync::Arc<str>, rue_rir::RirParamMode, Type, bool)>,
    pub result: Type,
    pub returns_borrow: bool,
    pub returns_inout: bool,
}

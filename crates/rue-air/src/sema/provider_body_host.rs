...
fn publish_provider_body_breakdown(
    ...
    tracing::event!(
        ...
        staged_frontier_bodies = expression.staged_frontier_bodies,
        staged_frontier_instructions = expression.staged_frontier_instructions,
        staged_fact_nodes = expression.staged_fact_nodes,
        staged_canonical_evaluations = expression.staged_canonical_evaluations,
        staged_constraints_generated = expression.staged_constraints_generated,
        staged_binding_scope_nodes = expression.staged_binding_scope_nodes,
        staged_binding_materializations = expression.staged_binding_materializations,
        ...
        staged_precompute_nodes = precompute
            ...
            .saturating_add(precompute.alias_allocations_examined)
            .saturating_add(precompute.inline_scan_pops)
            .saturating_add(precompute.inline_scan_child_edges)
...
pub struct ProviderBodyWork {
    ...
    /// Reachable selector frontiers generated during staged inference.
    pub staged_frontier_bodies: u64,
    /// Resolved instruction facts generated by staged frontiers.
    pub staged_resolved_instructions: u64,
    /// Fact nodes inspected by the canonical selector frontier.
    pub staged_fact_nodes: u64,
    /// Actual canonical selector/generic evaluation calls.
    pub staged_canonical_evaluations: u64,
    /// Constraints generated by probe and frontier passes (excluding final).
    pub staged_constraints_generated: u64,
    /// Persistent lexical scope nodes pushed during staging.
    pub staged_binding_scope_nodes: u64,
    /// Persistent scope lookup/materialization visits charged by staging.
    pub staged_binding_materializations: u64,
    /// Persistent trie path nodes written by lexical scope pushes.
    pub staged_binding_trie_updates: u64,
    /// Persistent trie levels consulted by runtime-name membership checks.
...
impl ProviderBodyWork {
    fn with_expression_breakdown(mut self, expression: ExpressionAnalysisBreakdown) -> Self {
        let precompute = expression.precompute_work;
        self.staged_probe_nodes = expression.staged_probe_nodes;
        self.staged_frontier_bodies = expression.staged_frontier_bodies;
        self.staged_resolved_instructions = expression.staged_frontier_instructions;
        self.staged_fact_nodes = expression.staged_fact_nodes;
        self.staged_canonical_evaluations = expression.staged_canonical_evaluations;
        self.staged_constraints_generated = expression.staged_constraints_generated;
        self.staged_binding_scope_nodes = expression.staged_binding_scope_nodes;
        self.staged_binding_materializations = expression.staged_binding_materializations;
        self.staged_binding_trie_updates = expression.staged_binding_trie_updates;
        self.staged_binding_trie_lookups = expression.staged_binding_trie_lookups;
        self.staged_precompute_nodes = precompute
            .alias_nodes_visited
            .saturating_add(precompute.alias_block_statements)
            .saturating_add(precompute.alias_allocations_examined)
            .saturating_add(precompute.inline_scan_pops)
...
struct ProviderBodyHost<'a, P, S, K, M> {
    ...
    /// (spec 6.7:15): header assertions from the subject's durable nominal
    ...
    M: Clone + Eq + Hash + Ord,
{
    ...
    /// that read its durable nominal (spec 6.7).
    ...
    M: Clone + Eq + Hash + Ord,
{
    ...
    fn inference_struct_type_by_file(&self, key: (FileId, Spur)) -> Option<Type> {
        // An interface shell is not a type (spec 6.7:18): inference gets no
    ...
    M: Clone + Eq + Hash + Ord,
{
    fn check_canceled(&self) -> CompileResult<()> {
        if self.provider.is_canceled() {
            Err(CompileError::without_span(ErrorKind::InternalError(
                "body analysis query canceled".to_owned(),
            )))
        } else {
            Ok(())
        }
    ...
    fn cancellation_probe(&self) -> Option<Box<dyn Fn() -> bool + '_>> {
        ...
        Some(Box::new(move || provider.is_canceled()))
    }

    fn sibling_attempt_hook(&self) -> Option<Box<dyn Fn() + '_>> {
        let provider = self.provider;
        Some(Box::new(move || provider.staged_sibling_attempt()))
    }
    ...
    M: Clone + Eq + Hash + Ord,
{
    ...
    fn conformance_assertions(&mut self, subject: Type) -> Vec<ConformanceAssertion> {
        ...
        // The subject's own header list (`struct S is I + J`, spec 6.7:9).
    ...
    fn interface_requirement_signature(
        ...
        for (index, parameter) in method.parameters.iter().enumerate() {
            ...
            params.push((parameter.name.clone(), mode, ty));
        }

//! Analysis context types for semantic analysis.
//!
//! This module contains the helper types used during function body analysis,
//! including variable tracking, move state, and scope management.
...
impl VariableMoveState {
    ...
    pub(crate) fn is_path_moved(&self, path: &[Spur]) -> Option<Span> {
    ...
    pub(crate) fn is_partially_moved(&self) -> Option<Span> {
    ...
    pub(crate) fn is_any_part_moved(&self) -> Option<Span> {
        if let Some(span) = self.full_move {
            return Some(span);
        }
        self.partial_moves.values().next().copied()
    ...
    pub(crate) fn merge_union(branch1: &Self, branch2: &Self) -> Self {
...
pub(crate) struct ParamInfo {
    ...
    pub(crate) abi_slot: u32,
    /// Parameter type
    pub(crate) ty: Type,
    /// Parameter passing mode
    pub(crate) mode: RirParamMode,
}
...
pub(crate) struct AnalysisContext<'a> {
    ...
    pub(crate) locals: HashMap<Spur, LocalVar>,
    /// Function parameters (immutable reference, shared across the function)
    pub(crate) params: &'a HashMap<Spur, ParamInfo>,
    ...
    pub(crate) used_locals: HashSet<Spur>,
    ...
    pub(crate) scope_stack: Vec<Vec<(Spur, Option<LocalVar>)>>,
    ...
    /// before AIR emission.
    pub(crate) resolved_types: &'a HashMap<InstRef, Type>,
    ...
    pub(crate) moved_vars: HashMap<Spur, VariableMoveState>,
...
impl AnalysisContext<'_> {
    ...
    pub(crate) fn insert_local(&mut self, symbol: Spur, var: LocalVar) {
    ...
    pub(crate) fn merge_branch_moves(
        &mut self,
...
impl AnalysisResult {
    #[must_use]
    pub(crate) fn new(air_ref: AirRef, ty: Type) -> Self {
        Self { air_ref, ty }
    }
}

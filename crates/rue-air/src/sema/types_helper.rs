//! Type-related helper methods for semantic analysis.
//!
//! This module contains methods for type resolution, type checking,
...
use crate::types::{ArrayTypeDef, ArrayTypeId, Type, parse_array_type_syntax};
...
impl Sema<'_> {
    ...
    /// Convert a concrete Type to an InferType.
    ///
    /// This handles array types by converting them to the structural
    /// InferType::Array representation.
    pub(crate) fn type_to_infer_type(&self, ty: Type) -> InferType {
        match ty {
            Type::Array(array_id) => {
                let array_def = &self.array_type_defs[array_id.0 as usize];
                InferType::Array {
                    element: Box::new(self.type_to_infer_type(array_def.element_type)),
    ...
    pub(crate) fn abi_slot_count(&self, ty: Type) -> u32 {
        match ty {
            ...
            // Array slot count is element slot count * length
            Type::Array(array_id) => {
                let array_def = &self.array_type_defs[array_id.0 as usize];
                let elem_slots = self.abi_slot_count(array_def.element_type);
                // Zero-length arrays have 0 slots
                elem_slots.saturating_mul(array_def.length as u32)
            }
        }
    }

    /// Get the slot offset for a specific field within a struct.
    ///
    /// Returns the sum of slot counts for all fields before the given index.
    pub(crate) fn field_slot_offset(
        &self,

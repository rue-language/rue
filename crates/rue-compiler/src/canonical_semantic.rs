...
/// it through [`BoundBodyEpoch::derive_body_epoch`]. The RUE-1133
/// clone-from-template probe (RUE-1091 ordered probe #1) instead deep-copies the
/// same epoch per body, so the derivation cost, the structural copy cost, and
/// the cost of sharing can be compared. Every arm consumes the epoch through
/// [`analyze_body_in_epoch`], so they differ in how the epoch was obtained and
/// in nothing else.

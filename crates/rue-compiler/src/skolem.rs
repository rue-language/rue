...
//! functions (spec 6.8:19–6.8:22, preview `interfaces`).
//!
...
//! requirements of the parameter's bound (spec 6.8:20). This module owns the
...
/// associated types (`T.Element`), itself an opaque skolem (spec 6.8:20).
...
impl SkolemIdentity {
    ...
    /// The name every diagnostic renders the skolem as (spec 6.8:22): the
...
fn type_names_skolem(ty: &TypeInstanceKey) -> bool {
    match ty {
        ...
        TypeInstanceKey::Function { params, result } => {
            params.iter().any(|(_, ty)| type_names_skolem(ty)) || type_names_skolem(result)
        }
    }
}

fn argument_value_names_skolem(value: &CanonicalArgumentValue) -> bool {
    match value {
        CanonicalArgumentValue::Type(ty) => type_names_skolem(ty),
        CanonicalArgumentValue::Function(function) => instance_is_skolem_check(function),
        CanonicalArgumentValue::Aggregate(value) => {
            type_names_skolem(&value.ty)
                || match &value.kind {
                    rue_air::CanonicalAggregateKind::Struct(values)
                    | rue_air::CanonicalAggregateKind::Array(values) => {
                        values.iter().any(argument_value_names_skolem)
                    }
                    rue_air::CanonicalAggregateKind::Enum { payload, .. } => {
                        payload.iter().any(argument_value_names_skolem)
                    }
                }
        }
        ...
        | CanonicalArgumentValue::Float(_)
        | CanonicalArgumentValue::Unit => false,
    }
}

/// Whether a body instance belongs to a skolem check (spec 6.8:22): the
    ...
    match instance {
        ...
        FunctionInstanceKey::Specialization { base, arguments } => {
            arguments.types.iter().any(type_names_skolem)
                || arguments.values.iter().any(argument_value_names_skolem)
        ...
        FunctionInstanceKey::ErrorPrinter(ty) => type_names_skolem(ty),
        FunctionInstanceKey::TestDispatcher => false,
    }
}
...
/// parameters are all type parameters (spec 6.8:19). A comptime value
...
/// (spec 6.8:24). Each check is the function's specialization by the
...
/// declare a bound (spec 6.8:25), so `all_modules` is false there and the
...
/// reports (spec 6.8:22), naming the function and the comptime parameters

...
const COVERAGE_MATRIX: [CoverageRow; 38] = [
    row!("RIR", "match arms"),
    row!("RIR", "directives"),
    row!("RIR", "parameters"),
...
fn owner_schema_inventories_are_complete_and_deliberate() {
    ...
    let mapped = COVERAGE_MATRIX
        ...
        .map(|row| row.family)
        .collect::<Vec<_>>();
    assert_eq!(&mapped[..18], EXPECTED_RIR);
    assert_eq!(&mapped[18..28], EXPECTED_AIR);
    assert_eq!(&mapped[28..], EXPECTED_CFG);

...
#
# Write mode only, and it stays a script for one reason: build and test actions
# must not mutate the source tree, so formatting-in-place cannot be a Buck
# target. Checking has no such constraint and is not here -- each crate emits
# its own `<name>-fmt-check` test from the rue_crate/rue_binary macros, so
# coverage follows the build graph instead of this script's file discovery
# (RUE-1152, RUE-1153).
#
...
if [ "$#" -gt 0 ]; then
    echo "fmt.sh: check mode has moved to the per-crate <name>-fmt-check tests." >&2
    echo "Run: ./buck2 test \$(./buck2 uquery \"kind('sh_test', 'root//crates/...')\" | grep -- -fmt-check\$)" >&2
    exit 2
fi

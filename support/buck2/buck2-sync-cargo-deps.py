...
# --- Minimal StrEnum back-port ------------------------------------------
class _StrEnumBase(str, Enum):
    """Behave like Python 3.11's enum.StrEnum on older versions.

    * members are instances of both `str` and `Enum`
    * str(member) → the underlying string value
    """

    def __str__(self) -> str:              # type: ignore[override]
        return str(self.value)

    def __format__(self, spec):            # keep f-string / format() parity
        return format(str(self), spec)
# ------------------------------------------------------------------------


class TomlSection(_StrEnumBase):

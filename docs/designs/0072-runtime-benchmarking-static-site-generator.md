...
static site generator written in Rue that builds a frozen snapshot of the
rue-lang.dev corpus. Gazette is measured whole-process by `rue-bench` on the
pinned CI runner regime, validated against byte-exact golden output, and
compared against pinned Zola (Rust) and Hugo (Go) building the same corpus
under a feature-parity configuration. The comparison question is order of
...
three tools are equivalent when they consume the same frozen corpus, are
configured to the same feature subset, and emit the same set of output files
with per-tool validated content. Byte-identical HTML across tools is a
non-goal: Zola, Hugo, and gazette use different Markdown renderers and will
disagree on whitespace and markup details. Within one tool, output is
byte-exact against a golden snapshot, so wrong-but-fast runs fail loudly.
...
### 2. Freeze the corpus, its template ports, and its scale variants

The runtime workload input is a byte-frozen snapshot under
`performance/workloads/`, following the same rules as the frozen lattice
compile workload: the snapshot does not track the live site, and a deliberate
corpus change requires a new suite revision re-pinned on every platform.

The frozen workload contains:

- the rue-lang.dev content tree as `website/build.sh` assembles it (site
  content plus the copied spec, ~96 markdown files, ~780 KB);
- template ports for each tool — gazette templates, a Zola template/config
  set, and a Hugo template/config set, each idiomatic to its tool; and
- deterministic 10x and 100x scale variants, generated by path-prefixed
  duplication of the 1x corpus, so results show a scale curve rather than a
  single point. The generator is checked in and deterministic; the variants
  are derived at fixture-preparation time, outside the measured window.
...
these features disabled; the parity configurations are part of the frozen
workload identity. The Tailwind step of the real site build is outside the
measured boundary for every tool.

Gazette's libraries reject constructs outside their documented subset with an
error rather than rendering them wrong, so future corpus drift is caught at
freeze time instead of corrupting output silently. Highlighting, search, and
...
- **Within a tool:** the output tree must match a byte-exact golden hash for
  the frozen corpus, per tool, recorded in the workload manifest.
...
Golden hashes are regenerated only with a suite revision. Cross-tool HTML
byte equality is explicitly a non-goal, per Terminology.
...
suite revisions pin workload identity (corpus hash, template ports, parity
configs, golden hashes, scale variants); epochs pin per-platform invocation,
sampling policy, and environment. This resolves RUE-1046's open CLI question:
...
commit, gazette source identity, and peer tool versions. In-process
...
Zola vs Hugo at each corpus scale), a longitudinal time series for gazette
across compiler versions, annotated events (compiler releases, peer tool
bumps, suite revisions), and side-by-side source and template excerpts. The
...
- [ ] **Phase 3: Gazette, the frozen corpus, output validation, and the
  scaling-curve rung** - RUE-1484
...
Phase 3's workload existing in frozen form and can overlap its later stages.
...
- Validation is honest: golden-output hashes and file-set equality make
  wrong-but-fast impossible to publish, and parity configs make the peers'
  job the same job.
...
- The corpus is self-renewing: as the real site grows, deliberate re-freezes
  scale the benchmark with it.
...
- Frozen corpus and template ports are maintenance surface: three template
  sets drift as tools evolve, and re-freezes require multi-platform re-pins.

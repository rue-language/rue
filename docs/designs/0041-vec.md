...
> **Amended by [ADR-0043](0043-collection-string-type-trio.md)**: `Vec` is renamed
> `ArrayBuf` and reframed as the growable rung of the collection trio
> (`[T; N]` / `borrow [T]` / `ArrayBuf(T)`). The mechanism described here — heap
> buffer, 2× growth, `drop fn` free — is unchanged.

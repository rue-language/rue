...
| **5** — Wrap single-nominal-identity, exit 42 | `wrap_single_nominal_identity_currently_fails_closed` → unit | PASS asserting the **fail-closed E9000**: the generic `Wrap` whose `get_or` method matches its anonymous-enum field `Option(T)` fails LOUD — error code `E9000`, message `…did not publish a terminal`, naming the reached method `get_or`. Never a silent miscompile. | **FLIPS**: replace the `Err` assertion with a compile+execute assertion → **exit 42**; the receiver field type, match enum key, payload op, and enum layout then all resolve to one `Option$…` nominal identity. Add the green spec case (template already inlined in the spec TOML). |
| **6** — both backends execute the Wrap payload regression | `wrap_payload_fails_closed_on_both_backend_targets` → unit | PASS asserting fail-closed **on both backend targets** (`x86-64-linux` and `aarch64-linux`): E9000 is a frontend failure reached before backend selection, so neither backend silently miscompiles the payload today. | **FLIPS**: compile with each `--target`, assert `Ok`; execute the native x86-64 ELF → exit 42; aarch64 stays a structural cross-compile check off its native host (mirroring `cli.abi_conformance`). Add two green `--target` CLI cases to `crates/rue-cli-tests/cases/`. |
| **7** — artificial anchor disagreement fails closed | *not encoded* | **Requires post-fix test hook — skipped per brief.** The anchor disagreement is exactly what naturally produces the E9000 in criteria 5/6 (astgen vs the durable fragment evaluator mint anonymous-type anchors independently). *Artificially injecting* a divergent anchor requires modifying the compiler minting sites (`SemanticConstEvaluator::eval_type_literal` / astgen); there is no public or test surface to force an artificial disagreement without touching compiler source. The natural fail-closed is already pinned by criteria 5 and 6. | Add a post-fix test hook that injects a divergent anchor and asserts the reconciliation still fails closed. |

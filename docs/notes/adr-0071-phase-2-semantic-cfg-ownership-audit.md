...
| Declaration facts: `SemanticNucleusValue` | `compiler.semantic-nucleus`; exact consumers are `CompilerBodyFactProvider`, type/layout/ABI/drop-glue families, the declaration aggregate, and producer/comptime queries. | One typed node per identity, signature, well-formedness, const/comptime call, anonymous nominal, or deferred-ownership key. Constant and comptime evaluation directly decode the candidate artifact with a borrowed packed-symbol view; no raw source fragment, lexer/parser, cloned AST, or mutable interner is reconstructed. Values use stable keys and mostly Arc-backed variable-width fields. Lattice body analysis performs 52,337 fact reads. Provider task caches retain terminals, but trait results are cloned by value. | Exact declaration key, target/preview configuration, and the precise lookup/candidate-artifact/nucleus dependencies observed by that projection. |
...
| Semantic body transaction: `BodyTransaction` and shared `CanonicalBody` | `compiler.body-transaction` owns analysis. Reachability reads its immutable `BodyReferences` directly; `compiler.body-produced-anonymous` remains the independently demandable producer projection, and `compiler.body-analysis-bundle` is the rooted closure's lifecycle aggregate over the transaction and that producer value. The redundant `compiler.body-references` and `compiler.canonical-body` registered projections are deleted. Live consumers are reachability, closure publication, warning production, and CFG input assembly. | The dependency-ready scheduler dispatches 1,263 Lattice transactions with zero serial fallback. A success owns one `Arc<CanonicalBody>` shared by the transaction, closure bundle, and `CfgBodyInput`; RUE-1346 made deep copies unrepresentable. The closure aggregate preserves its existing retention/invalidation boundary without a second canonical-body node. Body analysis nevertheless performs 61,474 name lookups and 19,211 owned durable materializations through `CompilerBodyDurableSource`. | Exact body key/configuration plus candidate artifact, signature, name/import, semantic nucleus, producer, type, and toolchain terminals actually observed. Production constant and comptime evaluation observe the same candidate artifact directly; the raw-constant family/materializer is deleted and the raw declaration-body materializer is `cfg(test)` only. Independent bodies retain independent invalidation. |

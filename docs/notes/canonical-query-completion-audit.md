...
| Pass/IR boundary simplification | Satisfied for production snapshot batch and semantic emits: both query a fresh `CanonicalFrontendSession`, and `CompilationUnit` remains only a compatibility facade. Compatibility projections are lazy and cannot trigger a second canonical lower. Declaration processing has explicit shell, resolution/import, and body-readiness phases. |
...
| Batch compatibility | Satisfied with deliberate diagnostic-order adapters. Snapshot batch uses the canonical session's parse/merge/lower/analyze queries, and parity tests compare artifacts, warnings, and failures. Durable projection tests compare ordinary and imported AIR epochs through bodies and CFGs. |

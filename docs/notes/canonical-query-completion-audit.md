...
| Pass/IR boundary simplification | Satisfied for production snapshot batch and semantic emits: both query a fresh `CanonicalFrontendSession`. The former `CompilationUnit` peer orchestrator and its compatibility projections have been removed. Declaration processing has explicit shell, resolution/import, and body-readiness phases. |

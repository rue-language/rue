...
Maintainer decision: this lands as an intentionally transitional coefficient
repair. It does not satisfy RUE-1135's provider-path identity condition because
the shared base still issues compact identities and tokens. The measured cold
win is useful on its own, but RUE-1092 must replace this boundary rather than
extend it.
...
the file/symbol path maps, the canonical import context, and the type pool's
symbol/language-item/repr side tables. Each sits behind an `Arc`, so a derivation
bumps a refcount instead of copying O(declarations) entries.
...
Focused ownership tests also compare the `Arc` identities for canonical-import
state and type-pool side tables. The latter test mutates a derived epoch and
checks copy-on-write isolation, so a sibling or the base cannot observe that
write. Their entries are included in `EpochDerivationUnits` rather than hidden
inside the aggregate shared count.
...
load-bearing. This approved implementation is therefore lateral to RUE-1092's
provider path, not a step on it. Removing the shared issuer requires the
provider-driven analyzer, which this change deliberately does not build.
RUE-1092 must delete this private request-scoped boundary outright rather than
make it authoritative or expose it as public API.

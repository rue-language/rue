...
A call's loan of a root spans the whole call, including the evaluation of its own argument list, so the rule also covers an exclusive loan taken *inside* another argument. An `inout` loan of the root taken anywhere within those argument expressions — as a nested call's argument, as a nested call's by-reference receiver, or within a block expression — overlaps the enclosing loan and **MUST** be rejected whenever the enclosing loan is either shared (`borrow`), or one whose operand undergoes view materialization (4.10:4) in either mode. A `str` operand is materialized as a two-word view when the argument is evaluated, so a nested call that reallocates or frees the buffer leaves that view referring to released storage; this is why an enclosing `inout str` conflicts even though it is itself exclusive. An enclosing **exclusive** loan whose operand is passed as an address does not conflict: the nested write reaches the same storage the enclosing callee reads, which is ordinary sequenced mutation, and the error-accumulator idiom `add(mul(x, y, inout flags), z, inout flags)` depends on that remaining legal. A nested loan of a *different* root never conflicts.

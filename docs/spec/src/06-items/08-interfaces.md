...
section requires `--preview interfaces` (6.8:3), except within the trusted
standard library (6.8:25).

## Preview gate

{{ rule(id="6.8:2", cat="syntax") }}

<!-- grammar-sync(id="6.8:2", production="interface_def", role="source") -->
<!-- grammar-sync(id="6.8:2", production="interface_list", role="source") -->
<!-- grammar-sync(id="6.8:2", production="interface_ref", role="source") -->
<!-- grammar-sync(id="6.8:2", production="interface_member", role="source") -->
<!-- grammar-sync(id="6.8:2", production="interface_const", role="source") -->
<!-- grammar-sync(id="6.8:2", production="interface_fn", role="source") -->
<!-- grammar-sync(id="6.8:2", production="interface_params", role="source") -->
<!-- grammar-sync(id="6.8:2", production="conformance_decl", role="source") -->
<!-- grammar-sync(id="6.8:2", production="interface_bound", role="source") -->
...
{{ rule(id="6.8:3", cat="legality-rule") }}
...
{{ rule(id="6.8:25", cat="legality-rule") }}
...
a trusted standard-library function passes the bound check of 6.8:15
...
definition-site check of 6.8:19 runs for those functions in every program.
...
module's user-defined type and interface names (6.0:2). It follows ordinary
directory-based visibility: access from another directory requires `pub`
(10.3).

{{ rule(id="6.8:5", cat="normative") }}
...
In this preview, method and associated-function requirements **MUST NOT**
declare comptime parameters or place-returning results (`-> borrow T` or
`-> inout T`). These forms require additional definition-checking support;
their qualifiers must not be discarded to obtain an ordinary signature.

{{ rule(id="6.8:6", cat="legality-rule") }}
...
{{ rule(id="6.8:7", cat="normative") }}

An interface **MAY** refine one or more other interfaces with
`interface Name: Parent + Other { ... }`. A type that conforms to a refining
interface must also conform to each refined interface (6.8:12). Refinement
**MUST** be acyclic.

{{ rule(id="6.8:8", cat="example") }}
...
{{ rule(id="6.8:10", cat="legality-rule") }}
...
- a method requirement is satisfied only by an inherent method of the same
  name whose receiver mode, parameter count, parameter modes, comptime status,
  parameter types, result mode, and result type equal the requirement's after
  substituting the asserted type for `Self` and the type's associated-constant values for the interface's
...
- a type-valued associated constant requirement is satisfied only by an
  ...
  type's body (6.8:2), whose right-hand side is a type.

An assertion is verified whenever it is relied on to satisfy a bound
(6.8:15); an implementation **MAY** also verify assertions eagerly. If any
...
{{ rule(id="6.8:12", cat="legality-rule") }}
...
verified by 6.8:10 as part of the same assertion.

{{ rule(id="6.8:13", cat="example") }}
...
{{ rule(id="6.8:15", cat="legality-rule") }}
...
interface refining it, 6.8:12) must occur in the program's selected import
graph. Assertions are program-global facts: the calling module need not
import the module containing the assertion. This does not change the
visibility rules for naming interface declarations or inherent members, and
does not discover assertions in files outside the selected graph. Otherwise
the call is a compile-time error at the call site that names the argument type, the parameter, and each
interface it does not conform to. Verification of the assertion itself
(6.8:10) is reported at the assertion, not at the call.

{{ rule(id="6.8:16", cat="normative") }}
...
In addition, the body is checked once against the bound itself (6.8:19), so
...
{{ rule(id="6.8:17", cat="example") }}

```rue
fn contains(comptime T: Equatable, borrow xs: ArrayBuf(T), borrow x: T) -> bool {
    let mut i: u64 = 0;
    while i < xs.len() {
        if xs.get_ref(i).equals(borrow x) { return true; }
        i += 1;
    }
    false
}
...
{{ rule(id="6.8:19", cat="normative") }}

For every module-level function that declares at least one interface-bounded comptime
...
`comptime T: A + B` bound to the *skolem type* of its bound set (6.8:20) and
...
interfaces of the bound set and the interfaces they refine (6.8:7). Its
...
`@align_of`, and `@offset_of` queries involving an opaque skolem **MUST** be
rejected, including queries reached through generic helpers. Likewise,
...
(compared as in 6.8:10); the skolem then has one member of that name.
...
{{ rule(id="6.8:22", cat="normative") }}

The skolem check is analysis-only. Every diagnostic it reports is placed at
the body span that produced it and renders the skolem as the parameter's name
(`T`), never as an internal name. A diagnostic from the bounded function's
own check carries the note
...
The check also analyzes generic helper bodies instantiated with skolems, so
calling a helper cannot bypass the bound's restrictions. Helper diagnostics
name opaque types such as `T.Element` at the helper's source location; the
preview does not attach the originating bounded function's note to every
helper diagnostic. These checks produce
no symbols and no code: neither a check nor its skolem-bearing callees reach
control-flow graph construction, code generation, or linking. Concrete
callees do not become executable roots through a check. In particular, the
bodiless requirements of an interface are never analyzed or emitted.

{{ rule(id="6.8:23", cat="example") }}
...
{{ rule(id="6.8:24", cat="normative") }}

Methods, associated functions, functions with comptime value parameters, and
comptime type constructors (functions whose result is `type`, 4.14) receive
no independent skolem check: their
...
function is instantiated. A helper method reached with opaque arguments from
a module-level function's skolem check is still analyzed under 6.8:22.

Recursive conformance proofs reached while resolving a requirement's bounded
type constructor are rejected with a compile-time diagnostic in this preview.
An in-progress verification is not evidence that the type conforms.

Associated types can appear by name in an interface requirement signature and
flow through inferred values. The diagnostic spelling `T.Element` is not a
supported qualified type expression in a generic body in this preview.

The preview also rejects bounded bodies whose helpers require a
source-visible layout for an opaque type. For example, growing an
`ArrayBuf(T)` reaches the storage helper's size and alignment queries.
Supporting this operation under a bound requires a separate account of
parametric storage; the preview does not treat a fabricated size as evidence
that such a body is valid for every conforming type.

## Name resolution

{{ rule(id="6.8:18", cat="normative") }}

An interface is named like a type: by its identifier in the declaring module,
or through a module binding (`m.Interface`, 10.4) under the ordinary
visibility rules (10.3). An

...
__rue_String_new(&mut s_storage);  // First arg is pointer to s_storage
...
#### `__rue_String_new`
```c
void __rue_String_new(StringResult* out);
```
...
```c
void __rue_String_with_capacity(StringResult* out, uint64_t capacity);
```
...
#### `__rue_String_from_str`
```c
void __rue_String_from_str(StringResult* out, const uint8_t* ptr, uint64_t len);
```
...
#### `__rue_String_len`
```c
uint64_t __rue_String_len(const uint8_t* ptr, uint64_t len, uint64_t cap);
```
...
```c
uint64_t __rue_String_capacity(const uint8_t* ptr, uint64_t len, uint64_t cap);
```
...
```c
uint8_t __rue_String_is_empty(const uint8_t* ptr, uint64_t len, uint64_t cap);
```
...
```c
void __rue_String_push_str(StringResult* out, const uint8_t* self_ptr, uint64_t self_len, uint64_t self_cap,
...
#### `__rue_String_clone`
```c
void __rue_String_clone(StringResult* out, const uint8_t* ptr, uint64_t len, uint64_t cap);
```
...
- `__rue_TypeName_method`: Method implementations for built-in types (e.g., `__rue_String_new`)

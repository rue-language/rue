...
| `RueCore/TraceOrder.lean` | **drop order**, `drop_order`, in two halves. Within a value, `run_blocks`: every run's trace is in the block grammar `Blocks`, each drop marker followed by exactly §6.11's walk of what it names, so every destructor runs inside the drop that owns it, in §6.11's order, and nowhere else (proved over `eval`, needing only `3.9:31`). Across cells, over §6's relation: `reachable_ordered`, every scope record of every reachable configuration is in location order, which is registration order, and `reachable_drop_order`, every reachable step's drops name one cell or distinct cells newest-first. Witnesses: `fieldsSwapped_rejected`, `returnPastAffine_newestFirst`, `unorderedRecord_rejected`, and the order-witnessing corpus cases read through the theorems | §3.9, §6.7, §6.9, §6.10, §6.11, §7 |

...
| `RueCore/TraceExact.lean` | the exact ledger `eval_exact`, the conservation law read as an equality over the identities an evaluation starts with, and `drop_exactly_once`: at every well-typed configuration of a checked program, every owned value ends exactly once — dropped, discarded or consumed on the normal or the unwind path, never both — with the `@panic` carve-out and the RUE-2316 one (`pendingSafe`, witnessed load-bearing by `pendingSafe_needed`) | §6.7, §6.9, §6.10, §7 |

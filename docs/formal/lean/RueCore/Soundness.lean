...
once" quantifies over. `dropContents_array_events` is the same read at an
...
(helper). -/
theorem ContentsTys.index : ∀ {D : Decls} {cs : List Contents} {Ts : List Ty}
...
(helper). -/
theorem ContentsTys.set : ∀ {D : Decls} {cs : List Contents} {Ts : List Ty}
...
theorem HasTy.contentsTy {D v T} (h : HasTy D v T) : ContentsTy D (Contents.ofVal v) T := by
  ...
  | array hvs => exact .array (HasTys.contentsTys hvs)
...
(helper). -/
theorem HasTys.replicate {D : Decls} {v : Val} {T : Ty} (h : HasTy D v T) :
    ∀ n, HasTys D (List.replicate n v) (List.replicate n T)
  | 0 => .nil
...
theorem dropContentsList_events {D : Decls} {cs : List Contents} {Ts : List Ty}
...
theorem dropContents_ok {D : Decls} {c : Contents} {T : Ty} (h : ContentsTy D c T) :
...
theorem dropContentsList_order {D : Decls} {c : Contents} {cs : List Contents}
...
theorem ContentsTy.residualLinear_false {D : Decls} {c : Contents} {T : Ty}
  ...
  | @array T n cs hcs =>
      ...
      intro T'' hmem
      have hT : T'' = T := List.eq_of_mem_replicate hmem
      subst hT
      intro hlin
...
theorem intResult_res {D : Decls} (w : IntWidth) (s : Sign) (n : Int) :
...
theorem binOpInt_res {D : Decls} (op : BinOp) (w : IntWidth) (s : Sign) (n₁ n₂ : Int)
...
theorem evalUnOp_int_res {D : Decls} (op : UnOp) (w : IntWidth) (s : Sign) (n : Int)
    (hop : op ≠ .not) :
...
theorem evalUnOp_bool_res {D : Decls} (b : Bool) :
...
(D-Int-Cast-Trap) say. -/
theorem evalIntCast_res {D : Decls} (w : IntWidth) (s : Sign) (w' : IntWidth) (s' : Sign)
    (n : Int) :
...
theorem evalUnOp_float_res {D : Decls} (w : FloatWidth) (f : FloatDatum) (hw : f.Wf w) :
...
theorem Ty.fieldAt_inv {D : Decls} {T Tf : Ty} {f : Nat} (h : T.fieldAt D f = some Tf) :
  ...
  | struct s =>
      ...
      split at h
      · exact Or.inl ⟨s, _, rfl, ‹_›, h⟩
      · cases h
  | array T' n =>
      -- A constant index step: `Ty.fieldAt` admits it exactly when `c < n`
      -- (`7.1:9`), and the element it reaches is the element type, which is
      -- `(List.replicate n T)[c]` — the same shape the struct arm hands the
      -- list lemmas.
      simp only [Ty.fieldAt] at h
      split at h
      · next hlt =>
...
theorem residualLinearFields_false {D : Decls} : ∀ {ts : List OwnSt} {Ts : List Ty}
...
(helper). -/
theorem dropRetire_ok {D : Decls} {H : Store} {ℓ : Nat} {cell : Cell} {c : Contents}
...
theorem OwnSt.join_matches {D : Decls} (hwf : WfDecls D) :
  ...
  | .fields as, .fields bs, e, T, c, hj, hc => by
      ...
      | array T' n =>
        simp only [OwnSt.join] at hj
        cases hjl : OwnSt.joinList D as bs (List.replicate n T') with
        | none => rw [hjl] at hj; cases hj
        | some es =>
            rw [hjl] at hj
            simp only [Option.map_some, Option.some.injEq] at hj
  ...
  | succ n ih =>
      ...
      | seq e₁ e₂ =>
          ...
          | affine =>
              ...
              cases hdv : dropContents P.decls (Contents.ofVal v) with
              | error w => rfl
              | ok evs =>

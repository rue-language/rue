...
def rows : List Row := [
  ...
  { thm := `RueCore.no_linear_leak
    ...
    diff := "Walker's rule is a property of the typing context. Ours is its dynamic image at the scope records: no scope exit, frame pop or unwind meets a live linear binding, as `eval`'s monitor for it watches. A linear operand value that a sibling's `return` or `break` abandons is outside it (RUE-2316)." },
  ...
  { thm := `RueCore.Config.trichotomy
    ...
    diff := "Immediate here, as in the literature. `Config.Stuck` is `step`'s `.stuck` verdict and `step` is total, so what the statement adds is that `step`'s `.halted` is `Config.Terminal` and its `.next` is a `Step`. That stuck means \"not terminal, and no rule applies\" is `Config.stuck_iff`; that `step` is `Step` is `step_iff`." },

...
Composed over a run this is an argument, not a theorem, and at `run` both
statements are vacuous (the run starts from the empty store and the entry
call's lead is empty). The whole-run statement is `whole_program_exactly_once`
(`TraceWhole.lean`, RUE-2478), proved directly by a ledger carried along
`eval`'s simulation rather than by composing the two.

...
unsafe def lintMain : IO UInt32 := do
  ...
  if sharp.isEmpty && spine.isEmpty then
    IO.println s!"- Sharpness: {RueCore.Spec.sharpness.length} counter-example statements (`RueCore.Spec.sharpness`) and {RueCore.Spec.sharpnessReasons.length} recorded reasons (`RueCore.Spec.sharpnessReasons`) cover every hypothesis of every spine statement (`Lint.hypotheses`), each counter-example its theorem's statement and bound in `RueCore.Spine`; each of the {(RueCore.Spec.sharpness.map (·.2.2.length)).sum} (counter-example, theorem, hypothesis) pairs is checked in the kernel by `RueCore.Sharp.Glue`, whose theorem states exactly the negation of the spine statement with that hypothesis dropped (`Lint.dropHyp`)."

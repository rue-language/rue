...
* `verdict` — `{"accept": {"type": <Rue type name>}}` when the checker
  ...
  (RUE-2330), and `return` or `@panic` only as the last form of an arm or of
  the function body (RUE-2383).
...
def cases : List Case := [
  ...
  { name := "two_params_dropped_at_pop",
    description := "Two by-value destructor-bearing arguments of two types the callee never consumes: the frame pop drops the last parameter first, so the trace is 2 and its field's 3, then 1, then the value 0.",
    rules := ["(Call) §5.8", "(D-Call) §6.9", "(D-Return-Value) §6.9", "§6.11"],
    prog := Examples.twoParamsDroppedAtPop },
  { name := "three_params_dropped_at_pop",
    description := "Three by-value destructor-bearing arguments the callee never consumes: the frame pop drops them last parameter first, 3, 2, 1, then the value 0.",
    rules := ["(Call) §5.8", "(D-Call) §6.9", "(D-Return-Value) §6.9"],
    prog := Examples.threeParamsDroppedAtPop },
  { name := "param_moved_other_dropped",
    description := "The first of two by-value parameters moved into a further call and the other left: the inner frame's pop drops the moved one (1), then the outer pop drops only the other (2), then the value 7.",
    rules := ["(Call) §5.8", "(Use-Move) §5.1", "(D-Call) §6.9", "(D-Return-Value) §6.9"],
    prog := Examples.paramMovedOtherDropped },
  ...
  { name := "destructure_array_residue",
    description := "An array in the residue: x.v selects past arr: [S1; 2], which split retains whole and drop* destroys at the access by §6.11's array rule, elements in ascending index order (3.8:73). A retained array is an ordinary residue place; a selected path through an index step is what needs §5.1's array clause, and that case is destructure_through_index.",
  ...
  { name := "array_elem_move_rest_ascending",
    description := "3.8:68's constant-index element move at the root binding: a[1] is moved out of an [S1; 3], its own drop runs where the @drop is, and the scope exit then walks the array 1, skip, 3 in ascending index order (3.8:73).",
    rules := ["(Use-Move) §5.1", "§6.11", "3.8:68", "3.8:73"],
    prog := Examples.prog Examples.tI64 Examples.arrayElemMove },
  { name := "array_elem_move_one_arm",
    description := "An element moved out in one arm of an if only: the §5.5 join meets MovedOut against Owned at the element and admits it because the element type is not Linear (3.8:50), so the scope exit drops only the sibling on the taken path.",
  ...
  { name := "array_elem_field_move",
    description := "A partial move at a path below a constant index, a[0].x0: 3.8:68 admits x[c].f... moves, the element keeps its Copy sibling, and the scope exit drops the element's remaining field record and the untouched element.",
    rules := ["(Use-Move) §5.1", "§6.11", "3.8:22", "3.8:68"],
    prog := Examples.prog Examples.tI64 Examples.arrayElemFieldMove },
  { name := "array_linear_elem_one_path",
    description := "A linear element consumed on one path only: 3.8:71 wants every element consumed on every non-diverging path, and the §5.5 join refuses at the element (E0443). The machine runs the taken path, which consumes both.",
    rules := ["(Use-Move) §5.1", "(If) §5.5 join", "3.8:50", "3.8:71"],
    prog := Examples.prog Examples.tI64 Examples.arrayLinearElemOnePath },
  { name := "array_declared_linear_elem",
    description := "An array of declared-linear elements with a[0].x0 selected: §4.2's dl puts the plan at the element, so the destructure consumes a[0] and leaves a[1] to an ordinary element move. 3.8:71's element-wise consumption, through both routes at once.",
    rules := ["(Use-Declared-Linear-Destructure) §5.1", "(Use-Move) §5.1", "3.8:33", "3.8:68", "3.8:71"],
    prog := Examples.prog Examples.tI64 Examples.arrayDeclaredElemDestructure },
  { name := "array_elem_reinit",
    description := "Reinitializing a moved-out element: 3.8:72 and 7.1:46 forbid assigning into an array with a moved-out element, at the moved index as much as at a sibling, because an element write does not reinstate per-element ownership (E0480). The whole-array reassignment is the recovery path.",
  ...
  { name := "array_dyn_read_below",
    description := "A Copy leaf read below a dynamic index, on an array binding and through a field: a[i].x1 + h.x0[i].x0 = 4 + 7 at i = 1 (probes q01, q08).",
    rules := ["(Use-Untrackable-Dynamic-Copy) §5.1", "(D-Index) §6.5", "7.1:10"],
    prog := Examples.dynReadBelow },
  { name := "array_dyn_write_below_affine",
    description := "A write below a dynamic index over a live affine leaf, a[i].x0 = S1 { 9 }: §6.8's overwrite-drop runs the old leaf's destructor at the assignment — 1, then 20, then the scope exit's 9 and 3, then 7 (probe q04).",
    rules := ["(Assign) §5.2", "(D-Assign) §6.8", "§6.8 overwrite-drop", "3.8:77"],
    prog := Examples.dynWriteBelowAffine },
  { name := "array_dyn_two_steps",
    description := "Two dynamic steps and a dynamic step under a constant place: a[i][0].x1 + a[0][i].x0 + a[i][i].x1 = 6 + 3 + 8 at i = 1 (probes q09, q10).",
    rules := ["(Use-Untrackable-Dynamic-Copy) §5.1", "(D-Index) §6.5"],
    prog := Examples.dynTwoSteps },
  { name := "array_dyn_read_trap_after_output",
    description := "@dbg(10); a[i].x1 at i = 1 and then at i = 5: the second read takes (D-Index-Trap) §6.5, and §6.12 keeps the 10, 4, 10 printed before it (probe q12).",
    rules := ["(Use-Untrackable-Dynamic-Copy) §5.1", "(D-Index-Trap) §6.5", "§6.12", "7.1:10"],
    prog := Examples.dynReadTrap },
  { name := "array_dyn_write_rhs_first",
    description := "a[id(1)].x0 = mk(9), both operands printing: 5.2:14 evaluates the right-hand side first and the target's index after it, so 9 prints before 1, and the overwrite-drop of the old S1 { 3 } comes after both (probes q14, q20).",
    rules := ["(Assign) §5.2", "(D-Assign) §6.8", "§6.2", "5.2:14"],
    prog := Examples.dynWriteRhsFirst },
  { name := "array_dyn_write_trap_negative",
    description := "a[i].x0 = S1 { 9 } at i = 0 and then at i = -1: the second write evaluates its right-hand side, finds the index out of range and traps, and the trap drops nothing — not the array and not the evaluated S1 { 9 } (probes q13, r08).",
    rules := ["(Assign) §5.2", "(D-Index-Trap) §6.5", "§6.12", "7.1:11", "5.2:14"],
    prog := Examples.dynWriteTrapNeg },
  { name := "array_dyn_read_after_sibling_move",
    description := "a[1] is moved out of an [[S8; 2]; 2] and dropped, and a[0][i].x1 then reads a whole a[0]: fully-owned is read at the array the dynamic step indexes, not at the root binding (probe r01).",
    rules := ["(Use-Untrackable-Dynamic-Copy) §5.1", "(Use-Move) §5.1", "3.8:68", "3.8:70"],
    prog := Examples.dynReadAfterSiblingMove },
  ...
  { name := "loop_moved_prev_iteration",
    description := "A loop whose body drops an outer affine binding and completes, so the back edge leaves it MovedOut, the loop-head state is the join of entry and back edge, MovedOut, and the @drop is refused at that head: moved in a previous iteration (3.8:79; the compiler reports E0205). The break-less loop is the let's whole body, so nothing follows it. The machine drops it on the first turn and meets the moved-out cell on the second.",
  ...
  { name := "repeat_copy_affine",
    description := "[e; n] of a non-Copy element: rule(id=\"7.1:38\") requires a Copy element type for the array-repeat literal (the compiler: E0905). The machine refuses the same repeat dynamically with typeConfusion, since the elaboration `let t = v; [t, t]` gets stuck at the second use of t. Seeded by the definition mutation analysis (RUE-2486): mutant `repeat-copy`.",
    rules := ["repeat arrays, elaborated to (Array-Intro) §5.8", "rule(id=\"7.1:38\")"],

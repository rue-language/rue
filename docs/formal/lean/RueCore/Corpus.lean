...
def cases : List Case := [
  ...
  { name := "array_write_after_destructure_via_field",
    description := "A declared-linear destructure at h.arr[0].x0 holes an array reached through a field, and a write through that element follows. 3.8:71/3.8:72 and 7.1:46 forbid it (E0480) for an array anywhere in a place tree. The bridge is red on this one: the compiler accepts it, runs the moved-out element's destructor twice and leaks the written value (RUE-2341). The case stays seeded until that is fixed.",

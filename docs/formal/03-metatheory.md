...
- **Statement, in words:** if `Typed P R Γ e T Ω` holds and the frame agrees
  ...
  unwinding `return`, a defined panic, or `outOfFuel`; it is never a
...
| Reading: `return` and `@panic` checked where they fire | **a reading §5.7's closing note allows**. `RueCore.Out` records no `⟨ret, _⟩` or `⟨panic, _⟩` delivery. (Fn) §5.8 consumes a `⟨ret, Σ_e⟩` only for its residual-linear check, and `RueCore.Typed.ret` carries that check as a premise at the edge; §5.7 exempts `⟨panic, _⟩` from §5.6, so nothing consumes it. The recorded deliveries are the `⟨break, Σ⟩` ones (Loop-Break) §5.7 joins; the fragment has no loop yet, so the list is always empty but threaded through every rule (RUE-2369 adds `loop` and `break`) |
| Reading: the diverge edge checked frame-wide (RUE-2369) | **accepted for the loop slice**. (Fn) §5.8 checks a `⟨diverge, Σ_h⟩` delivery against the by-value parameters; the loop slice will check it where it fires, against every binding in scope — the non-panic residual condition §5.6 and §5.7 retain for a `⊥_diverge` edge. The compiler agrees: a linear **local** live at `loop { }` is E0406, as a linear parameter is (probes recorded in RUE-2326's report) |
| Reading: `break` carries no value (RUE-2369) | **as the calculus writes it**. §2, §6.10 and `4.8:22` have no value-carrying `break` ("`break expr` is a compile-time error at the surface"), so the loop slice mechanizes the nullary `break` §2 writes |

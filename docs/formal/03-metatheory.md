...
| Associativity of §5.5's branch join | **checked, not proved** (RUE-2337). Commutativity *is* proved (`RueCore.OwnSt.join_comm`, lifted by `RueCore.Ctx.join_comm`), which is what makes a two-arm `if` join order-free. A `match` joins n-way, and the mechanization reads §5.5's unordered `join(Σ1, …, Σn)` as the **left fold** of the binary join in declaration order (`RueCore.Ctx.joinAll`), so associativity is what would make the bracketing immaterial and the fold invariant under a permutation of the arms. It is false in the generality the statement would need: `.fields` at a scalar type is a state no rule can write, `ownedJoinOk` refuses it while `residual-linear` sees nothing in it, and at `int` the two associations of `MovedOut`, `Owned`, `fields [Owned]` disagree — one is `MovedOut`, the other ill-formed (`lean/RueCore/Examples.lean` pins the pair). Over states that *are* well formed at their type it holds, checked exhaustively over a 13-declaration fixture and every well-formed `OwnSt` to depth 3, with no counterexample. Proving it needs a state-against-type well-formedness invariant the fragment does not carry today (`lean/RueCore/Statics.lean`'s join section) |

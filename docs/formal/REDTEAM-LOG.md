...
- **Trunk:** `c2fe428ff`, with the six seeds the pass adds. For the mutants
  those seeds could affect, the pass was also run without them.
...
- **Models:** one Claude Opus session wrote the mutants, ran the script and
  read each mutant against the stated properties. The reading is one of:
  - a stated property is false;
  - only a helper is false;
  - every statement holds;
  - equivalent.

  The readings and their reasons are in `mutate.py`'s `RULINGS`. A kill is a
...
Each is reproduced by `mutate.py --work <dir> --only <id>` on the tree above.
...
- `soundness`, `check_sound` and `checkProgram_sound`, against every mutant
...
- `Exact` (`drop_exactly_once`, `rest_exactly_once`), against:
  ...
  - a skipped `break` unwind;
  - the three trace-only mutants.
- `soundness`, against the join mutants (`join-owned-wins`,

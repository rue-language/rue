...
| orthogonal | Of the core: it has one way to express each idea | none | — | ours, pending audit | README “The architecture: surface → elaboration …”; MUTATION “Proposed issues” |
...
| prose specification; prose spec; spec; `X.Y:Z` | The human-facing Rue specification in `docs/spec/`, cited by paragraph id | [spec 1.3:1a][s1] | — | Rue-specific, grounded | 01 intro; README intro; REDTEAM “Roles”; lean/README “The bridge corpus”; GUIDE “How to read this guide”; BRIDGE-SENSITIVITY “Method”; MUTATION “What is mutated”; `Float` |
...
| MUST NOT; MUST | A requirement level: an absolute prohibition, an absolute requirement | [FIELD §7][F7]: RFC 2119 §§1–2; [spec 1.4:3][s1] | — | standard | 01 §2; 03 “Linear values are consumed exactly …”; README intro; REDTEAM “Roles”; lean/README “Generated programs”; GUIDE “Fuel, and why the theorems …”; BRIDGE-SENSITIVITY “Method”; MUTATION “RUE-2490's survivors”; `Float` |
...
| uninhabited | Of a type: it has no values, like an enum with zero variants | [spec 6.3:12][s6.3] | — | Rue-specific, grounded | 01 §2; MUTATION “RUE-2490: the statement vocabulary and …”; `Statics` |
...
| mutable; `mut`; unmarked | Of a place: it may be assigned, because its root is a `mut` binding or an `inout` parameter | [spec 6.1:43][s6.1] | — | Rue-specific, grounded | 01 §2; lean/README “Generated programs”; GUIDE §5; BRIDGE-SENSITIVITY “Follow-ups”; MUTATION “Reading a proof failure”; `Syntax` |
...
| projection; projected from | Selecting a field or element of a place (`p.f`, `p[e]`) | [spec 3.8:76][s3.8] (field projection) | — | Rue-specific, grounded | 01 §2; 03 intro; lean/README “Generated programs”; MUTATION “RUE-2490's survivors”; `Syntax` |
...
| ownership state; `Owned`; `MovedOut`; owned | The static record, per path, of whether a place owns its value or has been moved out | [spec 3.8:79][s3.8] (the ownership state of a binding); FIELD §5: `MovedOut` is "moved from" (clear), `Owned` partial ([FIELD §5][F5]) | `OwnSt` | Rue-specific, grounded | 01 §4.1; 03 intro; README “The extension rubric”; lean/README “Generated programs”; GUIDE §1; BRIDGE-SENSITIVITY “Results”; MUTATION “What the proofs kill, and …”; `Syntax` |
...
| derivation | A tree of rule instances that proves a judgment | [FIELD §9][F9]: PFPL ch. 2 | `Typed` | standard | 01 §3; 03 “No use-after-drop / no leak …”; lean/README “Explaining a program”; GUIDE §1; BRIDGE-SENSITIVITY “Rule coverage”; MUTATION “RUE-2490: the statement vocabulary and …”; `Syntax` |
...
| stuck; stuck configuration | Not terminal, and no rule applies | [FIELD §1][F1]: PFPL ch. 6; Plotkin §3.1 Def. 11 | `Config.Stuck` | standard | 01 §6.1; 03 intro; lean/README “Generated programs”; GUIDE §2; MUTATION “The mutants”; `Float` |
| stuck (the interpreter's refusal); monitored stuck | `eval`'s named refusal of a step, including the four checks §6 does not have | [FIELD §1][F1]: `EvalRes.stuck` differs, partial | `EvalRes`, `Violation` | ours, pending audit | 01 §6.1; 03 intro; lean/README “Generated programs”; GUIDE §2; MUTATION “The mutants”; `Float` |
...
| canonical | Of a finite datum's representation: the significand is odd or zero, so each number has exactly one | none | `canonNum`, `FloatDatum.Wf` | ours, pending audit | 01 §5.5; 03 intro; README “Contents”; REDTEAM “Roles”; lean/README “What is mechanized”; MUTATION “What is mutated”; `Float` |
...
| exact; rounded (operation) | An operation whose result needs no rounding, and one whose result is `rnd_w` of an exact value | [spec 3.12:36][s3.12] ("each result is exact") | `FloatOps` | Rue-specific, grounded | 01 §5; 03 “No use-after-drop / no leak …”; README intro; REDTEAM “The log”; lean/README “Deciding whether to believe it”; GUIDE §3; MUTATION “What is mutated”; `Float` |
...
| subnormal | A finite float below the smallest normal magnitude, with reduced precision | [spec 3.12:10][s3.12] | `FloatWidth.eMin` | Rue-specific, grounded | MUTATION “What is mutated”; `Float` |
...
| float model; `FloatModel`; interface | The rounded float operations, given as parameters with the laws IEEE 754 guarantees, rather than defined | none | `FloatModel`, `FloatOps` | ours, pending audit | 01 §6.4; 03 “Type safety”; README intro; lean/README “Deciding whether to believe it”; GUIDE §4; MUTATION “RUE-2490: the statement vocabulary and …”; `Float` |
...
| witness | A concrete program, with a checked proof about it, that shows a theorem's case happening | [FIELD §7][F7]: interesting witness (Beer et al. §4.3 Def. 20), partial (theirs is a model or path) | — | ours, pending audit | 01 §8; REDTEAM intro; lean/README “Layers”; GUIDE “The three trace theorems, one …”; MUTATION intro; `Float` |
...
| change | One lane's diff, what per-lane review attacks, as opposed to the whole claim | none | — | ours, pending audit | 01 §2; README “Relationship to the prose spec”; REDTEAM intro; lean/README “Explaining a program”; GUIDE “The three trace theorems, one …”; MUTATION intro; `Dynamics` |
...
| `↔`; `⟺` | "if and only if"; "to and from" | Logical equivalence; in prose, a conversion both ways (float↔integer) | Lean's `↔` (TPIL, [FIELD §8][F8]) | none | 01 §2; lean/README “Sharpness counter-examples”; `Statics` |
...
| `FloatDatum.Wf` | `Float` | A float value is canonical and, when finite, representable at the given width | ours, pending audit | MUTATION “What is mutated” |
...
| `FloatModel` | `Float` | The float operations together with the laws §7 needs of them (closure, totality), each true of IEEE 754 | ours, pending audit | 03 “Type safety”; README intro; lean/README “Deciding whether to believe it”; GUIDE “What the proof needs”; MUTATION “RUE-2490: the statement vocabulary and …”; `Float` |
...
| `run` | `Dynamics` | A program's outcome (§6.12): calls the entry function with no arguments in an empty store | ours, pending audit | 01 §6.9; 03 “Type safety”; lean/README “Explaining a program”; GUIDE §2; MUTATION “RUE-2490: the statement vocabulary and …”; `Dynamics` |
...
| `HasTy` | `Soundness.Defs` | Value typing: a machine value has a type (integer ranges, struct fields against the declaration) | standard (FIELD §2: typing judgment `⊢ v : τ`) | REDTEAM “Targets”; lean/README “What is mechanized”; GUIDE “What the proof needs”; MUTATION “What is mutated”; `Statics` |
...
| `Cons` | `Trace.Defs` | For one evaluation: the result's store, value and freed identities together own at most what the start owned | ours, pending audit | lean/README “What is mechanized”; MUTATION “RUE-2490's survivors”; `Trace.Defs` |

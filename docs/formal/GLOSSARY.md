...
| search; (Search) | The rule that finds the redex inside an evaluation context | [FIELD §1][F1]: PFPL §5.2 (search rules) | `Step` | standard | 01 §6.2; 03 “Lemmas §7 owes, and the …”; GUIDE “One program, traced both ways”; TOOLING “How it fits this package”; `Float` |
...
| scope record; scope records; `σ`; open scopes | Per frame, the list of cells each open scope must drop at its exit | none; FIELD §5 maps it to Rust's drop scope ([FIELD §5][F5]) | `Frame` | ours, pending audit | 01 §2; 03 intro; README “The extension rubric”; lean/README “Generated programs”; GUIDE §1; CHECKER-PROFILE “Why a rejected program still …”; `Float` |
...
| mint; mints; fresh; fresh cells | Creating a new allocation or value identity, one never used before | none | `mintParams`, `Fresh` | ours, pending audit | 01 §2; 03 intro; REDTEAM “Targets”; lean/README “The statement layer”; GUIDE §2; BRIDGE-SENSITIVITY “Rule coverage”; `Dynamics` |
...
| statement; statements | What a theorem asserts, read apart from its proof | none; FIELD §8: a theorem's type is a `Prop` (Reference §7.4) ([FIELD §8][F8]) | — | ours, pending audit | 01 §2; 03 “Type safety”; README intro; REDTEAM intro; lean/README intro; GUIDE “How to read this guide”; MUTATION intro; TOOLING “The workflows”; `Float` |
...
| locality; `Untouched` | An evaluation grows the store and changes no cell outside its frame's own | none | `Untouched` | ours, pending audit | 01 §5.7; 03 “Type safety”; lean/README “How to read this, with …”; GUIDE §3; MUTATION “The mutants”; `Statics` |
...
| module; `module` system | A Lean source file, and Lean's module system that controls what one exposes | [FIELD §8][F8]: Reference §§5.3–5.4 | — | standard | 01 §2; lean/README “Building”; GUIDE §2; MUTATION “Method”; CHECKER-PROFILE “Why a rejected program still …”; TOOLING intro; `Float` |
...
| `run` | `Dynamics` | A program's outcome (§6.12): calls the entry function with no arguments in an empty store | ours, pending audit | 01 §6.9; 03 “Type safety”; lean/README “Explaining a program”; GUIDE §2; CHECKER-PROFILE intro; `Dynamics` |

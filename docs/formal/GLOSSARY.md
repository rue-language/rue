...
| prose specification; prose spec; spec; `X.Y:Z` | The human-facing Rue specification in `docs/spec/`, cited by paragraph id | [spec 1.3:1a][s1] | — | Rue-specific, grounded | 01 intro; README intro; REDTEAM “Roles”; lean/README “The bridge corpus”; GUIDE “How to read this guide”; BRIDGE-SENSITIVITY “Method”; `Float` |
...
| intrinsic; `@drop`; `@panic`; `@dbg`; `@intCast`; float intrinsics `@f` | A built-in operation written with `@`, with its own typing and reduction rule | [spec 2.5:5][s2.5] | `FloatIntrin`, `Expr` | Rue-specific, grounded | 01 §2; 03 intro; README intro; lean/README “The bridge corpus”; GUIDE §2; BRIDGE-SENSITIVITY “Results”; `Float` |
...
| path; paths | The route from a place's root binding to it, as a list of field and index steps | [FIELD §5][F5]: Polonius (path); rustc-dev-guide (move path) | `Place.path` | standard | 01 §4.2; 03 intro; README intro; lean/README “Generated programs”; GUIDE §1; BRIDGE-SENSITIVITY “Results”; `Syntax` |
...
| evaluation context; evaluation contexts; `E` | An expression with one hole, placed where the next step happens | [FIELD §1][F1]: Felleisen & Hieb §2; PFPL §5.3 | `Kont` | standard | 01 §2; 03 intro; README “Relationship to the prose spec”; lean/README “The bridge corpus”; GUIDE §1; `Float` |
...
| fuel | A bound on how much work the interpreter may do; running out gives a timeout | [FIELD §3][F3]: Amin & Rompf | `eval` | standard | 01 §7; 03 intro; lean/README “The bridge corpus”; GUIDE §2; `Dynamics` |
...
| definitional interpreter; `eval` | An interpreter that serves as the definition of the language | [FIELD §3][F3]: Amin & Rompf §2.1 | `eval`, `run` | standard | 01 §6; 03 intro; README “The executable oracle”; REDTEAM “Targets”; lean/README “Explaining a program”; GUIDE §2; BRIDGE-SENSITIVITY “Rule coverage”; `Float` |
...
| run | A whole evaluation of a program from its initial configuration | [FIELD §6][F6]: run (Leucker & Schallhart) | `run` | standard | 01 §1; 03 intro; README intro; REDTEAM “Targets”; lean/README “The bridge corpus”; GUIDE intro; BRIDGE-SENSITIVITY “Method”; `Float` |
...
| shell | What remains of a matched enum value after its payload is moved out | none | `matchConsume` | ours, pending audit | 03 “No double-free”; lean/README “The statement layer”; GUIDE §2; `Dynamics` |
...
| float model; `FloatModel`; interface | The rounded float operations, given as parameters with the laws IEEE 754 guarantees, rather than defined | none | `FloatModel`, `FloatOps` | ours, pending audit | 01 §6.4; 03 “Type safety”; README intro; lean/README “Deciding whether to believe it”; GUIDE §4; `Float` |
...
| seed; seeds; seed cases; seed corpus | The hand-written corpus cases | [FIELD §7][F7]: "seed cases" differs, partial | — | ours, pending audit | 01 §6.4; 03 “Type safety”; lean/README “The bridge corpus”; GUIDE §4; BRIDGE-SENSITIVITY intro; `Checker.Defs` |
| seed (random); `--seed N` | The number that fixes a generated corpus | [FIELD §7][F7]: random seed (libFuzzer `-seed=N`; Csmith §3.4) | — | standard | 01 §6.4; 03 “Type safety”; lean/README “The bridge corpus”; GUIDE “Loops, briefly”; BRIDGE-SENSITIVITY intro; `Checker.Defs` |
...
| verdict | The checker's accept or reject on a corpus case | [FIELD §7][F7]: "verdict" differs, none | `checkProgram` | ours, pending audit | 03 “Lemmas §7 owes, and the …”; README “The executable oracle”; REDTEAM “Targets”; lean/README “The bridge corpus”; GUIDE §4; BRIDGE-SENSITIVITY “Rule coverage”; `Checker.Defs` |
...
| change | One lane's diff, what per-lane review attacks, as opposed to the whole claim | none | — | ours, pending audit | 01 §2; README “Relationship to the prose spec”; REDTEAM intro; lean/README “Explaining a program”; GUIDE “The program”; `Dynamics` |
...
| `FloatModel` | `Float` | The float operations together with the laws §7 needs of them (closure, totality), each true of IEEE 754 | ours, pending audit | 03 “Type safety”; README intro; lean/README “Deciding whether to believe it”; GUIDE “What the proof needs”; `Float` |
...
| `WfDecls` | `Statics` | The declarations are well formed: no containment cycle, and every struct and enum class is correct | standard (FIELD §9: well-formed, PFPL §1.2) | 03 “Type safety”; `Checker.Defs` |
...
| `eval` | `Dynamics` | The interpreter: runs an expression with a fuel bound, over a float model, to an `EvalRes` | standard (FIELD §3: definitional interpreter) | 03 intro; README “The executable oracle”; REDTEAM “Targets”; lean/README “Explaining a program”; GUIDE §2; BRIDGE-SENSITIVITY “Rule coverage”; `Statics` |
...
- In RUE-2241's Linux lane; The lint; On macOS; Read the claim; Running Comparator; unsandboxed and says nothing; `3.0:5` (E0483), mechanized; A `@panic` (by design); Accessor calls

...
Some numbering comes from a preprint, and those cases are flagged.
...
| terminal transition system | A transition system with a set T of final configurations, none of which can step | Plotkin 1981/2004, Def. 1.2.2 |
| small-step / big-step | Steps one atomic transition at a time / relates a term directly to its final result. Kahn named big-step "natural semantics" | Plotkin 2004b; SF *Smallstep* |
...
| standard reduction | A reduction step taken only inside an evaluation context | Felleisen & Hieb, Def. 2.3 |
| reduction semantics | The Felleisen school's name for semantics given as contexts plus reductions | Felleisen & Hieb (usage) |
| value; `e val` | A finished computation | PFPL §5.2; SF |
| normal form | A term that cannot step | SF *Smallstep* |
| stuck | Not a value (not final) and no step applies | Plotkin ch. 2; PFPL §5.1; Wright & Felleisen Def. 4.8; SF |
...
| evaluation dynamics | Harper's name for big-step evaluation `e ⇓ v` | PFPL ch. 7 |

### Symbols

| Symbol | Reading | Variants |
|---|---|---|
| `e ↦ e′` | "e steps to e′" | Pierce/SF `t → t′`; Plotkin `γ → γ′`. Felleisen & Hieb and Wright & Felleisen use `↦` for a step in context and `→` for the notion of reduction |
...
brackets; "value", "stuck" and "checked error" as PFPL defines them.
...
| Big-step = small-step | `e ⇓ v ⇔ e ↦* v ∧ v val` | PFPL Thm 7.2 (cited from the abbreviated edition) |
| Church–Rosser; standardization | Reduction is confluent; `e →* e′` iff a standard reduction sequence exists | Felleisen & Hieb Thms 2.2, 2.5 (credited to Plotkin) |
...
| trap `↯κ`, "defined panic" (§6.1, §6.12) | checked error `e err` (PFPL §6.3) | partial: the same role, but a different name and symbol |
...
| "hole" for the `⊘` cell content (`Contents.hole`, GUIDE §3, the §7 bullets) | "hole" is the empty position of a context. The accepted words for a moved-out cell are in §5 below | partial: a second meaning of an accepted term, and both meanings appear in GUIDE §2 |
| control stack `K`, `Kont`, `Focus` (§6.1, `Step.lean`) | abstract-machine vocabulary. No source in this map covers it | none (unverified) |
...
| subject reduction | Reduction preserves typing. The older name, from combinatory logic | Wright & Felleisen, Main Lemma 4.3 |
...
| syntactic type soundness | Soundness proved by progress and preservation | Wright & Felleisen; Timany §2.5 |
...
| compatibility lemma | The semantic counterpart of one typing rule | Timany §6 |
| fundamental theorem | Syntactic typing implies semantic typing | Timany Thm 6.5 |
| adequacy (of the semantic model) | Semantic typing implies `safe` | Timany §4.1, §6.10; RustBelt Thm 7.2 |
| step-indexing | Indexing the logical relation by a step count, which makes recursive types well founded | Appel & McAllester 2001; Timany §4.2 |
...
| `Σ ⊢ μ` | store typing | TAPL ch. 13 (the symbol comes from section titles only) |
...
| Name | Form | Source |
|---|---|---|
| Preservation | `Γ ⊢ e : τ ∧ e ↦ e′ ⇒ Γ ⊢ e′ : τ` | PFPL Thm 6.2; Wright & Felleisen Lemma 4.3 |
...
| Adequacy | `∅ ⊨ e : τ ⇒ safe(e)` | Timany §4.1 |
...
| `step_preservation` over `Config.SafeAt` | Preservation is syntactic (`⊢ e′ : τ`). A statement over "nothing reachable is stuck, and every halted value is typed" is **semantic type soundness** in Timany's safety form | partial: the name clashes with the accepted syntactic meaning |
| `Config.SafeAt` | `safe(e)` plus value typing: the conclusion of adequacy in Timany | clear |
| "fundamental lemma" `init_safeAt` (metatheory): a checked program's initial configuration is `SafeAt` | Timany's fundamental theorem is `⊢ ⇒ ⊨`. `init_safeAt` is `⊢ ⇒ safe`, the fundamental theorem composed with adequacy | partial |
| `FrameMatches` / `Matches` ("Σ faithfully tracks the store's initialization") | the invariant a syntactic proof carries: store typing / a well-typed machine state | partial: ours relates ownership state rather than types |
| Σ for the ownership state (§5) | Σ is store typing in TAPL and the global environment in Oxide | partial: symbol clash |
| `Violation` (`useAfterMove`, `useAfterDrop`, …) | `wrong` (Milner), "going wrong" (CompCert), stuck (PFPL) | partial: ours is a named refusal |
...
2024 (as a secondary source for what "adequacy" means).

### Accepted terms

| Term | Meaning | Source |
|---|---|---|
| natural semantics | Kahn's name for big-step operational semantics | Plotkin 2004b |
...
| definitional interpreter | An interpreter that serves as the definition of a language | Reynolds 1972; Amin & Rompf |
...
| computational adequacy | A denotational semantics agrees with an operational one on ground-type programs | Plotkin 1977, via Niu, Sterling & Harper |
...
| `a ⇒∞` | "a diverges" (big-step) | `(t,s) ⇑` (Owens); `e⇑` (Wright & Felleisen) |
...
line). The out-of-fuel result is a *timeout* and is kept apart from error;
...
| Name | Form | Source |
|---|---|---|
| Big-step/small-step equivalence (terminating runs) | `a ⇒ v ⇔ a →* v ∧ v value` | Leroy & Grall Thm 9 (§4) |
| Divergence equivalence | `a ⇒∞ ⇔ a →∞` | Leroy & Grall Thm 11 |
| `big_iff_small` | `cs ⇒ t ⇔ cs →* (SKIP, t)` | Isabelle HOL-IMP `Small_Step` |
| Semantic equivalence | `e →* e′ ⇒ ∃n. eval n e ∼ e′`, and `eval n e = r ⇒ ∃e′. e →* e′ ∧ r ∼ e′` | Amin & Rompf Thm 2 |
| Functional ⇔ relational big-step | `(t,s) ⇓ r ⇔ ∃c. sem_t (s with clock c) t = r ≠ Rtimeout`; always timing out ⇔ `⇑` | Owens §3.4 |
| Equivalence with small-step | the section title for relating the clocked interpreter to a small-step semantics | Owens §7 |
| Clock monotonicity | Not timed out at clock c ⇒ the same result at every c + k | Owens (unnamed); SF `ceval_step_more` |
| Soundness via a definitional interpreter | `⊢ e : T ∧ eval n e = r ≠ Timeout ⇒ r = Val v ∧ v : T` | Amin & Rompf Thm 1; Owens §5; Siek |

**The name of the eval ⇔ small-step theorem.** The dominant name is
**equivalence of big-step and small-step semantics**. Leroy & Grall §4 and
Owens §7 use it, and Isabelle's theorem is `big_iff_small`. For a fuelled
interpreter against a small-step relation, Amin & Rompf's **semantic
equivalence** is the closest verified form. "Adequacy" is not used for this
relation in any source here. Its accepted meanings are:
...
| Our term | Accepted term | Confidence |
|---|---|---|
| "adequacy", "the adequacy lemma", `Adequacy.lean` (`eval` ⇔ `Step*`) | equivalence of big-step (definitional-interpreter) and small-step semantics; "semantic equivalence" (Amin & Rompf) | partial: both accepted meanings of "adequacy" are different relations, and one of them (model adequacy) also appears in our §2 material |
| `eval_sound` / `eval_complete` | the two directions of the equivalence (`big_to_small` / `small_to_big` in Isabelle HOL-IMP; `eval__multistep` / `multistep__eval` in SF) | clear |
| "definitional interpreter" `eval` (ADR-0097) | definitional interpreter (Reynolds; Amin & Rompf) | clear |
| fuel, `.outOfFuel` | fuel (Amin & Rompf), clock (Owens); timeout | clear |
| `fuel_mono` | clock monotonicity | clear |
| `no_masking` (no fuel hides a violation) | follows from monotonicity. No separate name was found | none |
| `eval_diverges_iff` (out of fuel at every bound ⇔ §6 never halts) | divergence equivalence (Leroy & Grall Thm 11; Owens §3.4) | clear |
...
| unrestricted | All three rules: any number of uses | Walker (`un`); Tov & Pucella write `U` |
...
| dereliction | An unrestricted value may be used where a one-use value is expected | Tov & Pucella |
| multiplicity | An arrow or binder annotation drawn from {1, ω} | Linear Haskell |
| usage; semiring {0, 1, ω} | QTT's annotations, which form a semiring | Atkey; McBride ("rig") |
...
| `q₁ ⊑ q₂` | Walker: "q₁ is more restrictive"; `lin ⊑ un` | **Direction varies.** In Tov & Pucella, U is the bottom and A the top: more restrictive is higher |
| `Γ₁ ∘ Γ₂` | context split | `Γ₁ + Γ₂` (Atkey, Linear Haskell) |
...
| Name | Form | Source |
|---|---|---|
| Classification by structural rules | ordered ∅; linear {E}; affine {E, W}; relevant {E, C}; unrestricted {E, W, C} | Walker §1.1 |
| Progress, preservation (linear λ) | the standard forms | Walker Thms 1.2.11, 1.2.12 |
...
| `Copy` | unrestricted (`un`, `U`). "Copy" is Rust's trait name | partial |
| `Affine`, `Linear` with §3's rule glosses | affine, linear; §3 names contraction and weakening correctly | clear |
| "multiplicity lattice" (§3), `Mult` | a qualifier order (Walker, Tov & Pucella). "Multiplicity" (Linear Haskell) means a semiring on arrows, {1, ω} | partial: the word comes from a neighboring framework with a different structure |
...
| "infectious" (a struct takes its fields' join) | the join of qualifiers. No separate verified term | none |
...
*Drop elaboration*); the Polonius book (*Atoms*); the Rust Book §4.1.
...
| drop elaboration; static / dynamic / open drop | Rewriting drops into code guarded by flags; a drop's target is always initialized / may be uninitialized / may be partly initialized | rustc-dev-guide |
| substructural (context) | RustBelt's typing context is substructural. Rust's own documentation does not call Rust "affine" | RustBelt |
...
| Symbol | Reading | Variants |
|---|---|---|
| `Σ; Δ; Θ; Γ ⊢ e : τ ⇒ Γ′` | Oxide's typing: e has type τ, and the stack typing becomes Γ′ | earlier Oxide drafts omit Θ (not rechecked) |
...
| Adequacy | A semantically well-typed `fn` never reaches a stuck state | RustBelt Thm 7.2 |
| Syntactic type safety | Oxide is proved sound by progress and preservation (the numbered statements were not seen) | Oxide §1.2 |

### Terms we currently use that differ from this

| Our term | Accepted term | Confidence |
|---|---|---|
| "hole", `⊘`, `Contents.hole` (a moved-out cell) | moved from / deinitialized / uninitialized (Rust Reference); dead (Oxide) | partial: the accepted words exist, and "hole" already means something else (§1) |
...
| "residue" (what remains of an aggregate after a move out of it) | the initialized fields of a partially moved value. There is no accepted noun | partial |
...
| §6.11's drop order (destructor, then fields in declaration order, then array elements in ascending order) | drop glue; drop order (Ref. *Destructors*) | clear |
| the dynamic `⊘` skip during a drop walk | the job drop flags do: dynamic and open drops | partial: the same job, but the state lives in the cell rather than in a flag |
| "Owned", "Borrowed" states; `inout`/`borrow` parameters | owner; borrow / loan; unique / shared reference | partial |
| "law of exclusivity" (§5.4) | unique (`mut`/`uniq`) vs shared (`shr`/`shrd`) references | partial |
...
| Term | Meaning | Source |
|---|---|---|
| execution; partial execution | An infinite sequence of states; a finite prefix of one | Alpern & Schneider |
| run; trace | A possibly infinite sequence of states or events; an execution is a finite trace | Leucker & Schallhart |
| trace property | A set of traces | Clarkson & Schneider |
| safety property | Nothing bad happens: every violation has a finite prefix that no continuation can repair. The term is Lamport's | Lamport 1977; Alpern & Schneider §2 |
| liveness property | Something good eventually happens: every finite prefix can be extended to satisfy it | Alpern & Schneider §3 |
| bad prefix / good prefix | A finite prefix that decides violation / satisfaction | Leucker & Schallhart |
...
| monitorable | A monitor can still reach a verdict. This class is strictly larger than safety ∪ co-safety | Leucker & Schallhart (after Pnueli & Zaks) |
| execution monitor (EM); security automaton | An enforcement mechanism that watches the execution and halts the program before a violating step | Schneider 2000 |
...
| Symbol | Reading | Variants |
|---|---|---|
| `S^ω`, `S*` | infinite / finite sequences of states | `Σ^ω`, `Σ*`; Clarkson & Schneider write `Ψ_inf` |
| `σ ⊨ P` | "σ satisfies P", i.e. σ ∈ P | `T ⊨ P` iff `T ⊆ P` for trace sets |
| `σᵢ` / `σ[..i]` | the prefix of length i | Alpern & Schneider / Schneider 2000 |
...
| Name | Form | Source |
|---|---|---|
| Safety (formal) | `∀σ. σ ⊭ P ⇒ ∃i. ∀β. σᵢβ ⊭ P` | Alpern & Schneider §2 |
| Liveness (formal) | `∀α ∈ S*. ∃β. αβ ⊨ P` | Alpern & Schneider §3 |
| Decomposition | Every property is the intersection of a safety property and a liveness property | Alpern & Schneider Thm 1 |
| EM enforceability | A policy an execution monitor enforces is a safety property; the converse fails | Schneider 2000 §2 |
| Hyperproperty decomposition | Every hyperproperty is the intersection of a safety and a liveness hyperproperty | Clarkson & Schneider |

### Terms we currently use that differ from this

| Our term | Accepted term | Confidence |
|---|---|---|
| "monitor" (`linearLeak`, `linearOverwrite`, `linearDiscard`, `ownedUnderCopy`): a check the interpreter adds, which refuses the step | execution monitor (Schneider 2000): it halts before a violating step. In runtime verification a monitor only returns a verdict and does not enforce | partial: our use matches Schneider's enforcement sense, not the runtime-verification sense |
...
al. 2015; Chen et al. 2020 (survey); Disselkoen et al. 2024 (Cedar); Leroy
...
| randomized differential testing | Differential testing on random inputs. No result oracle is needed | Csmith §2.1; survey §4.1 |
| differential random testing (DRT) | Cedar's check that the Lean model and the production code agree on millions of generated inputs | Cedar §4 |
| verification-guided development | Write an executable model, prove properties of it, and check the product against it with DRT | Cedar §1 |
| executable model / specification | Cedar: the models serve as the specification | Cedar §3 |
| test oracle; oracle problem | A procedure that decides whether a test's behavior is correct; the difficulty of obtaining one | Barr Def. 2.4 |
...
| pseudo-oracle | An independently written version of the program, used as the oracle | Barr §5.1 |
| metamorphic testing; EMI | Relations between inputs and outputs; variants equivalent on the given inputs | Barr §5.2; survey §4.2 |
| random program generation; generator | Csmith is a randomized test-case generator | Csmith §2 |
| undefined-behavior avoidance | Generated programs must be well formed and have a single meaning | Csmith §2.2 |
| wrong-code bug | Silent miscompilation | Csmith §3 |
| test-case reduction | Heuristic shrinking of a failing input, deliberately not called minimization | C-Reduce §3.1 |
| property; generator (`Gen`, `Arbitrary`) | QuickCheck's vocabulary. "Shrinking" does **not** appear in the 2000 paper | QuickCheck §§2–3 |
| seed | Three senses: seed inputs / seed corpus (libFuzzer); PRNG seed (Csmith §3.4); seed program (survey) | as listed |
...
| certifying compiler | A compiler that emits a certificate alongside its code; only the checker is trusted | Leroy CACM §2.2; Necula §1 |
...
| `Safe(S)` | S cannot go wrong | — |
| `∼` | a simulation invariant | Necula writes Σ |
...
| Forward simulation | `∀B ∉ Wrong, S ⇓ B ⇒ C ⇓ B`. It implies backward simulation when C is deterministic | JAR Def. 5; CACM eq. 3 |
| Verified compiler / validator | `Comp(S) = OK(C) ⇒ S ≈ C` / `Validate(S,C) = true ⇒ S ≈ C` | CACM eqs. 6, 7 |
| DRT agreement | `model(x) = prod(x)` on every generated input x | Cedar §4 |
| QuickCheck property | `∀x. P x`, checked on random x | QuickCheck §2.1 |

**Where the bridge sits.** The bridge runs the compiler, the oracle and the
native binary on generated and hand-written programs, and compares their
outcomes with the Lean interpreter's. That is **differential random testing
against an executable model**: McKeeman's differential testing, and Cedar's
DRT almost exactly. The model serves as a specified test oracle (Barr). The
...
trusts the runner and the model.

### Terms we currently use that differ from this

| Our term | Accepted term | Confidence |
|---|---|---|
| "the bridge", "bridge corpus" (ADR-0097; [lean/README.md](lean/README.md)) | differential random testing (Cedar); randomized differential testing (Csmith) | partial: "bridge" is our word, and the activity has an accepted name |
| "differential-tested", `rue-oracle-diff` | differential testing | clear |
| "oracle" (`rue-oracle`, the executable reference interpreter) | test oracle, specifically a pseudo-oracle or specified oracle (Barr). The accepted term names the pass/fail judge, not the interpreter itself | partial |
| "executable reference interpreter" / "the executable semantics" (§6) | executable model / specification (Cedar) | clear |
| "seed cases" (the hand-written corpus cases) | seed inputs / seed corpus (libFuzzer) | partial: fuzzing seeds are the initial inputs that mutation starts from |
| `--seed N`, `gen_<seed>_<i>` | PRNG seed | clear |
| generated programs (`Gen.lean`) | random program generation | clear |
| "verdict" (the checker's accept or reject on a case) | a verdict is a monitor's output in runtime verification (§6) | partial |
| "model gap", "gap registry" (§6.13.6) | no verified counterpart | none |
| "red case", "disagreement" | candidate bug; discrepancy (McKeeman) | partial |
...
de Moura, Kong & Ullrich, *Theorem Proving in Lean 4* (TPIL); de Moura &
Ullrich 2021; the Lean API documentation (`Lean.ReducibilityAttrs`,
...
| lean4checker; comparator | Replays declarations through the kernel; compares a statement against an external checker | Reference, "Validating a Lean Proof" |
| axiom | A postulated constant. Axioms do not reduce, so a closed term that uses one can get stuck | Reference §8 |
| standard axioms | `propext`, `Classical.choice`, `Quot.sound` | Reference §8.4 |
| `funext` | A theorem, proved from `Quot.sound` | TPIL "Axioms and Computation" |
| `sorry` / `sorryAx` | A placeholder that closes any goal, implemented by the axiom `sorryAx` | Reference §8 |
...
| reducible / semireducible / irreducible | How eagerly a definition unfolds. Semireducible is the default | `Lean.ReducibilityAttrs` |
...
| inductive predicate | An inductive type in `Prop` | TPIL "Inductive Types" |
| `decide` / `native_decide` | Evaluates a `Decidable` instance in the kernel / in compiled code, which adds an axiom. Before 4.29 that axiom was `Lean.ofReduceBool`; from 4.29 each use gets its own | `Init.Tactics`; 4.29.0 release notes |
| `noncomputable` | Required on code that depends on `Classical.choice` | Reference §8; TPIL |

**"Definitional unfolding"** does not appear in the pages fetched. The accepted
phrasing is **definitional equality**, with **unfolding** (δ-reduction)
controlled by reducibility. Lean has no official symbol for definitional
...
| Axiom audit | `#print axioms T` ⊆ {`propext`, `Classical.choice`, `Quot.sound`}; any `sorryAx` means the proof is incomplete | Reference, "Validating a Lean Proof" |
...
| "trust report" (`TRUST.md`) | an axiom audit plus a statement of the trusted code base | partial |
| "definitional unfolding" (RUE-2459's own wording) | definitional equality; δ-unfolding; reducibility | partial |
...
| judgments as inductive types (GUIDE §1) | inductive predicates | clear |
...
or Crossref record was fetched but the full text was not, and no term in this
file is attributed to that source's content.
...
| Appel & McAllester 2001 | A. W. Appel, D. McAllester. An indexed model of recursive types for foundational proof-carrying code. *TOPLAS* 23(5):657–683, 2001 | https://doi.org/10.1145/504709.504712 | OpenAlex record and abstract |
...
| Barendregt & Wiedijk 2005 | H. Barendregt, F. Wiedijk. The Challenge of Computer Mathematics. *Phil. Trans. R. Soc. A* 363(1835):2351–2375, 2005 | https://doi.org/10.1098/rsta.2005.1650 | Crossref; cs.ru.nl PDF of the same title |
...
| Cedar 2024 | C. Disselkoen et al. How We Built Cedar: A Verification-Guided Approach. 2024 | https://arxiv.org/abs/2407.01688 | "How We Built Cedar: A Verification-Guided Approach" (the FSE venue was not shown on arXiv) |
| Chen et al. 2020 | J. Chen, J. Patra, M. Pradel, Y. Xiong, H. Zhang, D. Hao, L. Zhang. A Survey of Compiler Testing. *ACM Comput. Surv.* 53(1), Art. 4, 2020 | https://doi.org/10.1145/3363562 | Crossref; PDF of the same title |
...
| de Moura & Ullrich 2021 | L. de Moura, S. Ullrich. The Lean 4 Theorem Prover and Programming Language (System Description). CADE-28, LNCS 12699, 625–635, 2021 | https://doi.org/10.1007/978-3-030-79876-5_37 | Crossref; lean-lang.org/papers/lean4.pdf |
...
| Felleisen & Hieb 1992 | M. Felleisen, R. Hieb. The revised report on the syntactic theories of sequential control and state. *Theor. Comput. Sci.* 103(2):235–271, 1992 | https://doi.org/10.1016/0304-3975(92)90014-7 | OpenAlex record; full-text PDF of the same title |
...
| Milner 1978 | R. Milner. A theory of type polymorphism in programming. *J. Comput. Syst. Sci.* 17(3):348–375, 1978 | https://doi.org/10.1016/0022-0000(78)90014-4 | Edinburgh Research Explorer abstract page; OpenAlex |
...
| Nipkow & Klein | T. Nipkow, G. Klein. *Concrete Semantics*; Isabelle HOL-IMP theory `Small_Step` | http://concrete-semantics.org/ | "Concrete Semantics"; "Small-Step Semantics of Commands" |
| Niu, Sterling & Harper 2024 | Y. Niu, J. Sterling, R. Harper. Cost-sensitive computational adequacy of higher-order recursion in synthetic domain theory. 2024 | https://arxiv.org/abs/2404.00212 | arXiv abstract page |
...
| Plotkin 1981/2004 | G. D. Plotkin. A Structural Approach to Operational Semantics. DAIMI FN-19, Aarhus, 1981; *J. Log. Algebr. Program.* 60–61:17–139, 2004 | https://doi.org/10.1016/j.jlap.2004.05.001 | Crossref/OpenAlex record; the author's own edition |
...
| Rust Reference | *The Rust Reference*: Destructors; Expressions; Glossary; Patterns | https://doc.rust-lang.org/reference/destructors.html | "Destructors - The Rust Reference" (and "Expressions", "Glossary", "Patterns") |

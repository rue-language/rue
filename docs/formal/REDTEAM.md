...
| **Trusted base** (axioms, `set_option` escapes, the digest and trust generators, the statement/proof split's Comparator configuration) | Try to get a hole past the checks: a new axiom, `sorry` behind a macro, `native_decide`, a claim file that differs from what was proved | Whether the lint and the trust report catch each attempt | RUE-2457 (trusted-base lint) |
...
- **Red agent.** A fresh context, never the lane's implementer or reviewer.
  ...
  first pass lost several findings to this (see the log).
- **Cross-model auditor.** A model from a different family (Codex today),
  ...
  report. The review checkpoints RUE-2251 and RUE-2252 are passes of this kind.
- **Adjudicator.** The coordinator. A finding **counts** once it is
  ...
  what the calculus *should* say, it goes to Dorian or Steve. Counted findings
  are filed as issues; the red agents never write to Linear or the tree.
...
- Every pass is a lane under the loop's two-lane cap; the red agent and the
  auditor count against it like any other subagent.
...
split lands; the digest until then), the calculus §§5–7, the trust report,
and this log, including its open findings. Where: the Lean community Zulip

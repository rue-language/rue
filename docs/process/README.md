...
> version control, AI-assisted workflows, and merge-queue management. They are
...
This directory documents how the maintainers develop the Rue compiler and
...
Each step has a corresponding document in this directory. Some steps also have
assistant-specific commands; those commands are adapters to the documented
process, not additional sources of truth.

## Quick Reference

| Step | Document | Command (where available) | Purpose |
|------|----------|---------|---------|
| Plan | [planning.md](planning.md) | `/plan` (historical, assistant-specific) | Design features, create ADRs and issues |
| Implement | [implementation.md](implementation.md) | `/implement` (historical, assistant-specific) | Write code, tests, and spec updates |
| Review | [code-review.md](code-review.md) | `/code-review` (historical, assistant-specific) | Check quality before committing |
| Commit | [committing.md](committing.md) | `/commit` (historical, assistant-specific) | Create well-formed commits |
| Coordinate agent work | [agent-work-contract.md](agent-work-contract.md) · [template](templates/agent-task-contract.md) | - | Proposed shared contract for Codex and Claude |
...
The slash-command entries above are retained historical examples from one
assistant integration; they are not universal commands or an additional source
of truth. Follow the linked process document and the applicable `AGENTS.md`.
...
- **AI assistants**: Codex, Claude, and other tools may provide commands or
  adapters for this documented process.

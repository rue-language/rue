...
DEFAULT_SPEC_DIR = ROOT / "docs" / "spec" / "src"
INDEX_NAME = "INDEX.md"
...
# error, the same way an unknown rule label already is. It is a different
# regex on different files, so a section citation (`§6.4`) or a rule label
# (`(D-Foo)`) — which merely *contain* digits that could pass for one half of
# a paragraph id — are never mistaken for one: only `PARAGRAPH_CITATION`
# matches decide what counts as a paragraph citation in the first place.
SPEC_RULE_ID = re.compile(r'\{\{\s*rule\(id="([^"]+)"')
...
SYNTAX_FORMS: Dict[Tuple[str, str], Tuple[str, List[str], str]] = {
    ...
    ("p", "p [ e ]"): (
        "yes",
        ["Place.idx", "Expr.indexRead", "Expr.indexWrite", "Expr.indexDrop"],
        ...
        "(`Expr.indexRead`/`Expr.indexWrite`/`Expr.indexDrop`): a constant "
        "place, then one or "
        ...
        "(D-Index-Trap). (@Drop-Copy) §5.3 has no index premise, so `@drop` of "
        "a `Copy` place below a dynamic index is in too, with the read's "
        "premises, value `()` and the same bounds trap. A constant-index read, "
        "write and `@drop` of the whole "
        "array are in, and so are `3.8:68`'s element **move** at the root "
        "binding (`rootIdxOnly`), the `MovedOut` element state it leaves, and "
        "`3.8:73`'s path-specific element drop. A move or a `@drop` of an "
        "affine or linear place below a dynamic index, and a use under a "
        "declared-`linear` prefix there, have no rule in the calculus either "
        "(§4.2's `Untrackable` plans, E0904): those are refused by design, not "
        "missing",
    ),
    ("e", "lit"): (
        "yes",
...
def parse_spec_paragraph_ids(spec_dir: Path) -> set:
    """Every id `docs/spec/src`'s ``{{ rule(id="X.Y:Z", …) }}`` shortcode declares.

    Every Markdown file under ``spec_dir`` is scanned for the shortcode; the
    id is whatever it names, letter-suffixed forms (`9.2:6b`) and appendix
    chapters (`A.2:1`) included, since this is a set membership test rather
    than a re-derivation of the numbering scheme.
    """
    ids: set = set()
    for path in sorted(spec_dir.rglob("*.md")):
        for match in SPEC_RULE_ID.finditer(path.read_text(encoding="utf-8")):
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--spec-dir", type=Path, default=DEFAULT_SPEC_DIR, help="docs/spec/src")
    ...
    def validate() -> Iterable[str]:
        modules, calculus, errors = collect(args.lean_dir, args.calculus, args.spec_dir)
        if errors:
            return errors

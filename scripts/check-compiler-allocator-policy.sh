...
require 'alias = "rue_alias"' "$third_party_root/reindeer.toml"
require 'load(":reindeer_rules.bzl", "rue_alias", "rue_rust_library")' "$third_party_root/reindeer.toml"
require 'def rue_alias(name, **kwargs):' "$reindeer_rules"
require 'def _is_linux_mimalloc_target(name):' "$reindeer_rules"
require 'kwargs["compatible_with"] = ["prelude//os:linux"]' "$reindeer_rules"
